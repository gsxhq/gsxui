...
func newMux(root string) http.Handler {
	...
	mux.HandleFunc("GET /f/chart-narrow-flex", func(w http.ResponseWriter, r *http.Request) {
		var buf bytes.Buffer
		if err := ChartNarrowFlexFixture().Render(r.Context(), &buf); err != nil {
			http.Error(w, "render: "+err.Error(), http.StatusInternalServerError)
			return
		}
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		if err := renderShell(w, "chart-narrow-flex", stylesheetFor(r), "/ui/index.js", template.HTML(buf.String())); err != nil {
			log.Printf("rendering chart-narrow-flex fixture: %v", err)
		}
	})
