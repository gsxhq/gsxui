...
func TestInputTypeOverridable(t *testing.T) {
	...
	// not duplicated.
	got := render(t, input.Input(gsx.Attrs{{Key: "type", Value: "email"}}))
...
func TestInputCallerClassMerges(t *testing.T) {
	got := render(t, input.Input(gsx.Attrs{{Key: "class", Value: "h-12"}}))
...
func TestInputAttrsFallThrough(t *testing.T) {
	got := render(t, input.Input(gsx.Attrs{{Key: "id", Value: "email"}, {Key: "placeholder", Value: "you@example.com"}, {Key: "disabled", Value: true}}))
