...
func TestInputOTPPinned(t *testing.T) {
	...
	want := `<div class="gap-2 flex items-center has-disabled:opacity-50" dir="ltr" data-gsxui-slot-input-otp><input inputmode="numeric" autocomplete="one-time-code" class="cursor-text disabled:cursor-not-allowed" data-gsxui-slot-input-otp-input>x</div>`
	if got != want {
...
func TestInputOTPCallerClassMerges(t *testing.T) {
	...
	if !strings.Contains(got, "max-w-sm") || strings.Count(got, "class=") != 2 {
		t.Errorf("caller class must merge onto the real input exactly once\nin: %s", got)
	}
}
...
func TestInputOTPGroupPinned(t *testing.T) {
	...
	want := `<div class="has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive rounded-lg has-aria-invalid:ring-3 flex items-center" dir="ltr" data-gsxui-slot-input-otp-group>x</div>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
...
func TestInputOTPSlotPinned(t *testing.T) {
	...
	want := `<div data-active="false" class="dark:bg-input/30 border-input data-[active=true]:border-ring data-[active=true]:ring-ring/50 data-[active=true]:aria-invalid:ring-destructive/20 dark:data-[active=true]:aria-invalid:ring-destructive/40 aria-invalid:border-destructive data-[active=true]:aria-invalid:border-destructive size-8 border-y border-r text-sm transition-all outline-none first:rounded-l-lg first:border-l last:rounded-r-lg data-[active=true]:ring-3 flex relative items-center justify-center data-[active=true]:z-10" data-gsxui-slot-input-otp-slot></div>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
...
func TestInputOTPSeparatorPinned(t *testing.T) {
	...
	want := `<div role="separator" class="[&amp;_svg:not([class*=&#39;size-&#39;])]:size-4 flex items-center" data-gsxui-slot-input-otp-separator><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-gsxui-slot-icon><path d="M5 12h14"/></svg></div>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
...
func TestInputOTPGroupCallerClassMerges(t *testing.T) {
	...
	if !strings.Contains(got, "gap-1") || strings.Count(got, "class=") != 1 {
		t.Errorf("caller class must merge onto the group exactly once\nin: %s", got)
	}
}
