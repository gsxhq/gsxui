...
func TestInputOTPPinned(t *testing.T) {
	...
	want := `<div class="gap-2 flex" dir="ltr" data-gsxui-slot-input-otp><input inputmode="numeric" autocomplete="one-time-code" class="cursor-text disabled:cursor-not-allowed" data-gsxui-slot-input-otp-input>x</div>`
	if got != want {
...
func TestInputOTPGroupPinned(t *testing.T) {
	...
	want := `<div class="has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive rounded-lg has-aria-invalid:ring-3 flex" dir="ltr" data-gsxui-slot-input-otp-group>x</div>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
...
func TestInputOTPSeparatorPinned(t *testing.T) {
	...
	want := `<div role="separator" class="[&amp;_svg:not([class*=&#39;size-&#39;])]:size-4" data-gsxui-slot-input-otp-separator><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-gsxui-slot-icon><path d="M5 12h14"/></svg></div>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
