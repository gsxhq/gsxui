...
func drawerContentClass(side string) string {
	...
	case "left":
		return base + " inset-y-0 left-0 right-auto h-full max-h-none w-3/4 rounded-r-xl border-r sm:max-w-sm data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left md:[&amp;_[data-gsxui-slot-drawer-header]]:text-start"
	case "right":
		return base + " inset-y-0 right-0 left-auto h-full max-h-none w-3/4 rounded-l-xl border-l sm:max-w-sm data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right md:[&amp;_[data-gsxui-slot-drawer-header]]:text-start"
	}
