/* Adapted from templui/shadcn-templ v2 chart.js (github.com/templui/templui @
 * 9ec720c03909, MIT) — see NOTICE.md.
 *
  ...
  (m.pies || []).forEach((pie, pi) => {
    ...
    for (let i = 0; i < sectors.length; i++) {
      ...
      for (const sh of shapes) {
        svg += `<path class="recharts-sector"${stroke}${strokeWidth} fill="${fill}" data-gsxui-chart-pie="${pi}" data-gsxui-chart-sector="${i}" d="${sectorPath(cx, cy, sh.inner, sh.outer, sc.start, sc.end)}"/>`;
      }
      svg += `</g>`;
    }
...
function tooltipWrapper(container) {
  ...
  // class name onto an element, unconditionally, on the theory that every
  // class ever attached to an element is server-rendered or a compiled
  // recipe string threaded through from one — this wrapper has neither, it
  // is a pure internal bookkeeping marker with no CSS rule keyed on it (the
  // reference's own "recharts-tooltip-wrapper" class was never a
  // Tailwind/recipe class either — the wrapper's actual presentation is the
  // inline style below).
  let wrapper = container.querySelector(":scope > [data-gsxui-chart-tooltip]");
  if (!wrapper) {
    wrapper = document.createElement("div");
    wrapper.setAttribute("data-gsxui-chart-tooltip", "");
...
/* renderChart is the module's one export, called by ui/chart.js's stub
 * once per [data-gsxui-slot-chart] container (gsxui.js's init() lifecycle:
 * current matches at registration, matches added later, and matches whose
 * subtree is morphed back to server state — see that module's own header).
 * container is already resolved by that caller, unlike the reference's own
...
function initPanel(container, script) {
  ...
  const render = (alpha = 1) => {
    ...
    if (state.points) container._gsxuiActivePoints = state.points;
  ...
  const enter = () => {
    const prev = container._gsxuiActive;
    const prevPoints = container._gsxuiActivePoints;
    container._gsxuiActive = m;
    const sameShape =
      m.kind === "pie"
  ...
  const handleMouseMove = (e) => {
    if (m.kind === "pie") {
      ...
      const idx = sector ? parseInt(sector.getAttribute("data-gsxui-chart-sector") || "-1", 10) : -1;
      if (idx < 0) {
        wrapper.style.visibility = "hidden";
        return;
      }
      positionTooltip(e, null, null, idx, parseInt(sector.getAttribute("data-gsxui-chart-pie") || "0", 10));
      return;
    }
