...
import (
	...
	"github.com/gsxhq/gsxui/ui/tooltip"
)
...
func TestTooltipStructure(t *testing.T) {
	got := render(t, tooltip.Tooltip(
		gsx.Fragment(
			tooltip.TooltipTrigger(gsx.Raw("Hover me"), nil),
			tooltip.TooltipContent(gsx.Raw("Add to library"), nil),
		),
		nil,
	))
...
func TestTooltipTriggerType(t *testing.T) {
	got := render(t, tooltip.TooltipTrigger(gsx.Raw("x"), nil))
...
func TestTooltipContentCallerClassMerges(t *testing.T) {
	...
	got := render(t, tooltip.TooltipContent(gsx.Raw("x"), gsx.Attrs{{Key: "class", Value: "z-10"}}))
...
func TestTooltipAttrsFallThrough(t *testing.T) {
	got := render(t, tooltip.TooltipContent(gsx.Raw("x"), gsx.Attrs{
		{Key: "id", Value: "my-tip"},
...
func TestTooltipPopoverAttrOverridable(t *testing.T) {
	...
	got := render(t, tooltip.TooltipContent(gsx.Raw("x"), gsx.Attrs{{Key: "popover", Value: "auto"}}))
