...
func TestToggleOffPinned(t *testing.T) {
	...
	want := `<button type="button" data-slot="toggle" data-gsxui-toggle data-variant="default" data-size="default" data-state="off" aria-pressed="false" class="inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap transition-[color,box-shadow] outline-none hover:bg-muted hover:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[state=on]:bg-accent data-[state=on]:text-accent-foreground dark:aria-invalid:ring-destructive/40 [&amp;_svg]:pointer-events-none [&amp;_svg]:shrink-0 [&amp;_svg:not([class*=&#39;size-&#39;])]:size-4 bg-transparent h-9 min-w-9 px-2">Bold</button>`
	if got != want {
...
func TestTogglePressedPinned(t *testing.T) {
	...
	want := `<button type="button" data-slot="toggle" data-gsxui-toggle data-variant="default" data-size="default" data-state="on" aria-pressed="true" class="inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap transition-[color,box-shadow] outline-none hover:bg-muted hover:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[state=on]:bg-accent data-[state=on]:text-accent-foreground dark:aria-invalid:ring-destructive/40 [&amp;_svg]:pointer-events-none [&amp;_svg]:shrink-0 [&amp;_svg:not([class*=&#39;size-&#39;])]:size-4 bg-transparent h-9 min-w-9 px-2">Bold</button>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
...
func TestToggleSizes(t *testing.T) {
	...
	for _, want := range []string{`data-size="sm"`, "h-8 min-w-8 px-1.5"} {
	...
	for _, want := range []string{`data-size="lg"`, "h-10 min-w-10 px-2.5"} {
	...
	for _, want := range []string{`data-size="default"`, "h-9 min-w-9 px-2"} {
