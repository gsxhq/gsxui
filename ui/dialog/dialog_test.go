...
func TestDialogPinned(t *testing.T) {
	...
	// and the Overlay's bg-black/50 is folded into backdrop:bg-black/50 on
	// the native ::backdrop (Portal/Overlay replaced by the top layer).
	got := render(t, dialog.DialogContent(false, dialog.DialogTitle(gsx.Raw("Title"), nil), nil))
	want := `<dialog data-slot="dialog-content" data-gsxui-dialog-content data-state="closed" class="fixed top-[50%] left-[50%] z-50 open:grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background p-6 text-foreground shadow-lg duration-200 outline-none sm:max-w-lg data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 backdrop:bg-black/50"><h2 data-slot="dialog-title" class="text-lg leading-none font-semibold">Title</h2><button type="button" data-slot="dialog-close" data-gsxui-dialog-close aria-label="Close" class="absolute top-4 right-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&amp;_svg]:pointer-events-none [&amp;_svg]:shrink-0 [&amp;_svg:not([class*=&#39;size-&#39;])]:size-4"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button></dialog>`
	if got != want {
