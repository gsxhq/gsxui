...
import (
	...
	"github.com/gsxhq/gsxui/ui/switchctl"
)
...
func TestSwitchCallerClassMerges(t *testing.T) {
	got := render(t, switchctl.Switch(gsx.Attrs{{Key: "class", Value: "w-12"}}))
...
func TestSwitchAttrsFallThrough(t *testing.T) {
	got := render(t, switchctl.Switch(gsx.Attrs{{Key: "id", Value: "s1"}, {Key: "name", Value: "notify"}, {Key: "aria-label", Value: "Notifications"}}))
...
func TestSwitchCheckedAttr(t *testing.T) {
	got := render(t, switchctl.Switch(gsx.Attrs{{Key: "checked", Value: true}}))
	if !strings.Contains(got, " checked") || strings.Contains(got, `checked="`) {
		t.Errorf("checked attr should render bare, not stringified\nin: %s", got)
	}

	got = render(t, switchctl.Switch(gsx.Attrs{{Key: "checked", Value: false}}))
...
func TestSwitchDisabledAttr(t *testing.T) {
	got := render(t, switchctl.Switch(gsx.Attrs{{Key: "disabled", Value: true}}))
