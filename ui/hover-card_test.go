...
func TestHoverCardPinned(t *testing.T) {
	...
	want := `<div class="contents" data-gsxui-slot-hover-card><span data-gsxui-slot-hover-card-trigger>Open</span><div popover="manual" data-state="closed" data-side="bottom" class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 bg-popover text-popover-foreground w-64 rounded-lg p-2.5 text-sm shadow-md ring-1 duration-100" data-gsxui-slot-hover-card-content>Body</div></div>`
	if got != want {
