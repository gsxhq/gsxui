...
import (
	...
	"github.com/gsxhq/gsxui/ui/tabs"
)
...
func TestTabsStructure(t *testing.T) {
	got := render(t, tabs.Tabs("a", gsx.Fragment(
		tabs.TabsList(gsx.Fragment(
			tabs.TabsTrigger("a", true, gsx.Raw("Tab A"), nil),
			tabs.TabsTrigger("b", false, gsx.Raw("Tab B"), nil),
		), nil),
		tabs.TabsContent("a", true, gsx.Raw("Content A"), nil),
		tabs.TabsContent("b", false, gsx.Raw("Content B"), nil),
	), nil))
...
func TestTabsTriggerSelectedStamping(t *testing.T) {
	active := render(t, tabs.TabsTrigger("a", true, gsx.Raw("x"), nil))
	for _, want := range []string{
		`data-state="active"`,
		`aria-selected="true"`,
		`tabindex="0"`,
	...
	inactive := render(t, tabs.TabsTrigger("a", false, gsx.Raw("x"), nil))
	for _, want := range []string{
		`data-state="inactive"`,
		`aria-selected="false"`,
		`tabindex="-1"`,
...
func TestTabsContentSelectedStamping(t *testing.T) {
	active := render(t, tabs.TabsContent("a", true, gsx.Raw("x"), nil))
	...
	inactive := render(t, tabs.TabsContent("a", false, gsx.Raw("x"), nil))
...
func TestTabsCallerClassMerges(t *testing.T) {
	got := render(t, tabs.TabsTrigger("a", false, gsx.Raw("x"), gsx.Attrs{{Key: "class", Value: "gap-4"}}))
...
func TestTabsAttrsFallThrough(t *testing.T) {
	got := render(t, tabs.Tabs("a", gsx.Raw("x"), gsx.Attrs{{Key: "id", Value: "t1"}, {Key: "aria-label", Value: "settings"}}))
...
func TestTabsTriggerPinned(t *testing.T) {
	...
	got := render(t, tabs.TabsTrigger("a", true, gsx.Raw("Account"), nil))
