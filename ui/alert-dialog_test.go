...
func TestAlertDialogActionPinned(t *testing.T) {
	...
	want := `<button data-variant="default" data-size="default" type="button" data-gsxui-slot="button" data-slot="alert-dialog-action" data-gsxui-dialog-close="true">Continue</button>`
	if got != want {
		t.Errorf("pinned render mismatch\n got: %s\nwant: %s", got, want)
	}
}
