...
## combobox
The Base UI (`@base-ui/react`) filtered listbox (`registry/new-york-v4/ui/combobox.tsx`) — NOT the legacy Command+Popover "combobox" recipe some registries ship under the same word. Parts: `Combobox`/`ComboboxInput`/`ComboboxTrigger`/`ComboboxClear`/`ComboboxContent`/`ComboboxList`/`ComboboxItem`/`ComboboxGroup`/`ComboboxLabel`/`ComboboxEmpty`/`ComboboxSeparator`/`ComboboxValue`. Built on `ui/combobox.js`, which restates `ui/select.js`'s value model/form bridge/popover machinery for an input trigger and layers on a `command.js`-style focus-stays-in-the-input highlight cursor.
