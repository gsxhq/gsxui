...
- GAP (DATA IS DROPPED — `mode="multiple"` submits only the first selected date): the hidden-input form bridge renders exactly ONE `<input type="hidden" name={name}>` for `single` AND `multiple` mode (`ui/calendar.gsx`'s `showHiddenSingle`/`hiddenSingleValue`, mirrored by `ui/calendar.js`'s `syncHiddenInputs`, which writes `selected[0]`). In multiple mode the full selection lives on the root as `data-gsxui-calendar-selected` (a comma-separated ISO list) and in the `gsxui:change` event's own `detail.selected` array, but a native form submission carries the FIRST date only: pick three days and two of them are silently discarded on submit. There is no error, no warning, and no truncation marker — a caller reading the posted value gets a well-formed single date and no signal that it is incomplete. Not fixed in the final review's fix wave: the correct shape is one input per selected date, which requires `calendar.js` to CREATE and DESTROY input elements as the selection changes, and this port's whole client contract is that JavaScript writes `data-*`/`textContent`/`tabindex`/`aria-*`/`disabled` on server-rendered elements and never adds or removes one. Changing that is a design decision, not a bug fix. Until it is made: bind `mode="multiple"` calendars through the `gsxui:change` event, or read `data-gsxui-calendar-selected` off the root — do not rely on the posted field. `single` and `range` (which renders its own `name`/`name+"-to"` pair) are unaffected.
