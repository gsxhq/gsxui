...
- ADAPT (cell-level `range_start`/`range_middle`/`range_end`/`hidden` className slots are not ported): react-day-picker's own `classNames.range_start`/`range_middle`/`range_end` (`rounded-l-md bg-accent` / `rounded-none` / `rounded-r-md bg-accent`, source map §2) attach to the `<td>` cell. gsxui's DOM contract puts every range/selection data attribute on the `<button>`, not the cell (`data-range-start`/`data-range-middle`/`data-range-end`, already live via `calendarDayButtonClass`) — there is no cell-level `data-range-*` attribute to gate an equivalent cell class on, and adding one solely to reintroduce this slot would contradict the fixed DOM contract task 1 established. The button fills the cell edge-to-edge, so the range band stays visually continuous without it. `hidden` (`invisible`) is likewise not ported — react-day-picker uses it to remove a day from the visual grid entirely (e.g. a padding day when `showOutsideDays=false`) while keeping its cell present for layout; no `data-hidden` attribute exists in gsxui's DOM contract for a class to key off. **Found while writing this entry, recorded rather than silently fixed:** `ui.Calendar`'s own `showOutsideDays` parameter is not actually wired to anything — every grid cell renders regardless of its value (`dayOutside` only ever feeds `data-outside`'s styling, never a render-or-not decision), so passing `showOutsideDays={false}` currently has no observable effect at all. Every shipped example passes `showOutsideDays={true}`, which is why this has gone unnoticed; the parameter exists (it's in the fourteen-parameter signature the design doc calls for) but is presently a no-op.
