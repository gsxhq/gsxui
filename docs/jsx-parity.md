...
- ADAPT (`ComboboxInput`'s attrs split class vs. everything else): the source destructures `{className, ...props}` — `className` merges into `cn("w-auto", className)` on the wrapping `InputGroup`; every other extra prop spreads onto the input itself. gsx's single `attrs` bag doesn't destructure on its own, so `ComboboxInput` does it explicitly: `attrs.Class()` reaches `InputGroup` (aggregated with the `"w-auto"` default via the ordinary tailwind-merge precedence), `attrs.Without("class")` reaches the input — the identical wrapper/control split `## native-select`'s own `NativeSelect` doc comment documents. `data-slot="input-group-input"` (not `InputGroupInput`'s own default `"input-group-control"`) is this port's own naming call for the actual text control, since Base UI's own ARIA anatomy for this part has no traced source to copy a slot name from (`derived-not-read` — `@base-ui/react` is absent from the checkout).
- MECHANISM (`ComboboxInput`'s addon inlines the trigger/clear buttons, doesn't call `ComboboxTrigger`/`ComboboxClear`): the source polymorphically merges `ComboboxTrigger`'s props onto `InputGroupButton`'s own `<button>` via Base UI's `render` prop (Radix's `asChild` cousin) — one DOM button, two logical components layered on it. gsx has no clone-element merge, so nesting two real component calls here would emit an invalid button-in-button. `ComboboxInput` instead builds the single button directly (`InputGroupButton` composed with `data-slot="combobox-trigger"`/`data-slot="combobox-clear"` passed as explicit non-parameter attributes — the same override mechanism `## input-group`'s own `InputGroupButton` doc comment documents for `data-size`). The standalone `ComboboxTrigger`/`ComboboxClear` parts still exist and render their own single `<button>` each, for direct use by the "trigger a popup from a button" composition variant (`combobox-popup.tsx`, out of this port's shipped examples) — they are simply never called BY `ComboboxInput` itself.
