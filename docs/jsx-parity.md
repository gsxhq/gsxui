...
## CSS boundary (shipped 2026-07-27)

Current gsxui components emit one bare
`data-gsxui-slot-<name>` presence marker per styling role and no presentation
classes. Composed roles forward as ordinary distinct attributes on the same
element. `assets/css/foundation.css` owns behavior mechanics,
`assets/css/themes/default.css` owns semantic values, and
`assets/css/styles/default.css` owns the default presentation. Older entries
below preserve `data-slot` and utility strings when they describe the
...
## animations
- FINDING (2026-07-24, fixed): every `animate-in`/`animate-out`/`fade-*`/`zoom-*`/`slide-in-*` token carried by dialog, dropdown, and tooltip was silently dead from v1 — those utilities are NOT Tailwind v4 core; they come from the `tw-animate-css` package, which shadcn's v4 globals.css imports and ours didn't. Ported classes pinned byte-for-byte can still be inert if the keyframes layer behind them never ships: class-string tests, `make check`, and even open/close behavior checks all pass without it (the components work, they just snap). Fixed by importing `tw-animate-css` from `assets/css/index.css`; getting-started now tells consumers to install it. Verified live: computed `animation-name` was `none` before, keyframed after.
- FINDING (2026-07-24, Tier 2 browser pass, fixed): dialog.js's animated close originally awaited every descendant `finished` promise, so an unbounded child animation (ui.Spinner's `animate-spin` is `iterations:Infinity`) could leave the dialog open forever in `data-state="closed"`. Close now observes only the dialog element's own finite animations and waits for all of those promises to settle; infinite child and dialog animations are ignored, canceled finite animations settle normally, and there is no duration cap. A backgrounded tab therefore completes when its real finite animation clock resumes. Applies to all three `<dialog>` consumers (dialog, alert-dialog, sheet).
...
- FIX (Header alignment, 2026-09-14 — a real drift, not a ledgered divergence): every style's `.gsxui-recipe-alert-dialog-header` carries upstream's `sm:group-data-[size=default]/alert-dialog-content:place-items-start` / `:text-left` pair, but `AlertDialogContent` never stamped `data-size`, so the selectors compiled and never matched: the alert dialog rendered upstream's `size="sm"` centred layout at every width in all eight styles, where shadcn's default (`size="default"`) is left-aligned from `sm` up. `AlertDialogContent` now stamps `data-size="default"` (the one value gsxui renders, since `size` itself is not ported); `TestAlertDialogContentStampsDefaultSize` pins it. This supersedes the ADAPT below, which described the centred rendering as deliberate.
- ADAPT (Header layout, superseded by the FIX above — kept for the history of the port): `AlertDialogHeader`'s `grid grid-rows-[auto_1fr] place-items-center gap-1.5 text-center` base — two grid rows (title then description, in DOM/source order; `grid-auto-flow`'s default `row` placement needs no explicit `grid-row` on either child), centered on both axes — renders identically at every viewport width in this port, since the one thing that would left-align it at `sm`+ (the `size=default`-conditional pair dropped above) is out of scope. Visually verified via `site/examples/alertdialog/basic.gsx`'s rendered markup: title stacks directly above description, both horizontally centered, no left-alignment breakpoint — matches shadcn's own `size="sm"` appearance at every width (the one variant this port effectively always renders, since `size` itself isn't ported) rather than its `size="default"` (the actual shadcn default, left-aligned at `sm`+) — a real, deliberate visual GAP downstream of the `size`-prop scoping decision above, not a copy error.
...
- WIN (presence-marker composition): `AlertDialog`, `AlertDialogContent`, `AlertDialogAction`, and `AlertDialogCancel` forward ordinary attributes into `Dialog`, `DialogContent`, or `Button` and add their own distinct `data-gsxui-slot-<name>` marker after `{ attrs... }`. The composed component keeps its own marker too, so one final element can carry, for example, both `data-gsxui-slot-alert-dialog-action` and `data-gsxui-slot-button`. There is no generic scalar `data-slot` override, `attrs.Has("data-slot")` guard, or slot-merging helper in the current contract.
- MECHANISM: `AlertDialogTrigger` renders its own `<button>` rather than composing `DialogTrigger` — same button-in-button HTML trap, same fix (`## dialog` FINDING): children must be phrasing content, and a styled trigger should be one `<ui.Button>` carrying `data-gsxui-dialog-trigger`, `data-gsxui-slot-alert-dialog-trigger`, and the trigger's static ARIA attributes directly, not a `Button` nested inside this wrapper. The site example (`site/examples/alertdialog/basic.gsx`) demonstrates the recommended idiom rather than the wrapper, mirroring `ui/dialog`'s own example.
...
- MECHANISM: `AspectRatio` parses the CSS `aspect-ratio` grammar (`auto || <ratio>`, where `<ratio>` is `<number> [ / <number> ]?`) and branches on the recognised form, instead of wrapping the value in `gsx.RawCSS`. gw's CSS value filter — a port of `html/template`'s — rejects `/` in an interpolated value, and `/` is `aspect-ratio`'s own required `<width> "/" <height>` separator, so the whole declaration previously had to be marked raw to get one character through. Each branch now writes the separator and the `auto` keyword as static template text and leaves each author-supplied number in its own `css\`\`` hole, where the filter still judges it: no hole can contribute a `/`, so none can open a CSS comment or start a second declaration, and nothing is trusted that was not first recognised as a number. A ratio the grammar does not cover is emitted unparsed and the filter alone decides — safe values render, hostile ones become the inert `ZgotmplZ`.
...
- ADAPT: `Avatar`'s `size` prop (default/sm/lg) is dropped along with it, so
  ...
  directly.
- FIX (theme-creator-parity Task 4, found reviewing the expanded preview by eye): `AvatarFallback`'s own class carried colour (`bg-muted`/`text-muted-foreground`) and `rounded-full` but never `size-full`/`items-center`/`justify-center` — as a flex child with no explicit size, it shrank to fit its text instead of filling `Avatar`'s circle, and that text was never centered within it. Invisible at the default `size-8` (close enough to look right by coincidence) but glaring at any larger caller override. `registry/canonical/avatar.gsx` now carries `size-full items-center justify-center` as a literal fragment alongside the recipe accessor, the same shape `Progress`'s own FIX above uses.
...
- ADAPT (nova, destructive variant, dark hover): nova's `destructive` recipe now includes `dark:hover:bg-destructive/90` (`registry/styles/nova/button.css`), where upstream shadcn's `buttonVariants` keeps `dark:bg-destructive/60` unchanged through hover — dark-mode destructive buttons never lighten on hover upstream. This is a deliberate presentation decision, not a token-parity fix: `jstest/harness/style_contract.gsx:63` already asserted `bg-destructive/90` under the `destructive-hover` reference name before this branch, so the recipe was brought into line with a contract this repo had already committed to. See `CHANGELOG.md` for the user-facing note. `ui/button_test.go`'s pinned-render test note has been corrected accordingly — the default button pin is no longer "no ADAPT deviations apply."
...
- ADAPT: `data-orientation` is still stamped on `ButtonGroup`'s root via the house `|> default("horizontal")` pattern (see button.gsx/dropdown-menu.gsx), for consistency with every other data-variant stamp in this codebase. shadcn's own `data-orientation={orientation}` leaves the attribute entirely unset when `orientation` is undefined (no defaulting on the raw prop, only inside the cva call) — a narrow, cosmetic divergence.
...
## calendar
- ADAPT (react-day-picker is not vendored — the month engine is ours): shadcn's `calendar.tsx` (`docs/superpowers/plans/2026-07-25-calendar-source-map.md`) wraps `react-day-picker`'s `DayPicker`, which owns date math, selection, and keyboard behavior internally. gsxui has no equivalent dependency to wrap, so `monthGrid` — the 42-cell six-week grid builder — is a from-scratch Go implementation in `ui/calendar.gsx`, with a byte-for-byte JS twin in `ui/calendar.js` (same contract, same UTC-throughout arithmetic, month 0-based in JS to match `Date`'s own convention). JavaScript constructs UTC dates through `utcDate`/`setUTCFullYear`, not `Date.UTC`: ECMAScript remaps numeric years 0–99 to 1900–1999, while Go preserves the exact year. The two must produce identical grids for the same `(year, month, weekStartsOn)` input — `jstest/specs/calendar.spec.ts`'s `"Go and JS agree on …"` tests (a family covering 28-day February, leap February, a month starting exactly on the week start, a month needing all 6 rows, a year boundary, and exact years 0000/0099, plus dedicated loaded/loaded-range variants for selection, disabled rules, and range flags) navigate client-side to a month and diff the resulting 42-cell state against the server's own render of that same month. This is the single largest risk this port carries (Task 6 review, fix round 1: an init-time repaint bug once made every agreement test compare JS to JS, masking a real JS-only `monthGrid` off-by-one for a full review round) and the reason the test exists at all, rather than trusting the two implementations to stay in sync by inspection.
- ADAPT (fixed six-row grid, where react-day-picker sizes four to six): react-day-picker's own `getMonths.js` builds each month's grid from that month's real first/last week (4–6 rows) and only pads to a fixed 42-cell grid when the caller opts into `fixedWeeks: true` — not the default. gsxui always renders six rows. Reason: a fixed cell count lets `calendar.js` update the 42 existing day cells' `textContent`/`data-*` in place on every navigation, never adding or removing a cell — which is what keeps every class string authority in `ui/calendar.gsx` alone (`.gsx` is what Tailwind scans; `.js` never builds a class string) and what lets focus survive navigation (the same `<button>` element that had focus before a prev/next click still has it after, just repainted to a different day). Consequence: the grid has constant height regardless of how many weeks a given month actually spans — a real, visible divergence from upstream's own variable-height months, not just an implementation detail.
- ADAPT (`captionLayout="dropdown"` defaults its year range): react-day-picker requires the caller to supply `startMonth`/`endMonth` for a dropdown caption — without them, `getYearOptions` returns `undefined` and the year `<select>` has no options at all (source map §7.4). `ui.Calendar`'s `fromYear`/`toYear` are optional; `calendarNavBounds` (`ui/calendar.gsx`) defaults them to current year −100 / +10 when zero, so `captionLayout="dropdown"` alone is never a broken caption. The `dropdown` example (`site/examples/calendar/dropdown-menu.gsx`) passes `fromYear`/`toYear` explicitly with a realistic birthdate-picker-shaped span; `bounded.gsx` separately pins both to the *same* year to make the navigation bound reachable in a handful of clicks for its own browser test.
- ADAPT (the `dropdown` className slot is dropped, along with `caption_label`'s dropdown-layout arm): react-day-picker's own dropdown caption is a semi-transparent-overlay trick — `Dropdown.js` layers a visible, `aria-hidden` `<span>` showing the selected option's label plus a chevron (styled via `caption_label`'s `captionLayout!=="label"` ternary arm) over a second, fully invisible `<select>` (`classNames[UI.Dropdown]`, source map §2: `absolute inset-0 bg-popover opacity-0`). gsxui's `captionLayout="dropdown"` instead renders `ui.NativeSelect` as-is — a real, always-visible, styled `<select>` (`ui/calendar.gsx:130-141`) — so there is no second invisible control and no faux label span for either the `dropdown` slot or `caption_label`'s dropdown arm to attach to; both are dropped, not silently carried. Consequence (Task 3, `.superpowers/sdd/2026-07-25-calendar/progress.md:30`): the dropdown caption renders at `NativeSelect`'s own generic sizing rather than nova's compact caption scale (`.cn-calendar-caption-label`'s `h-6 pr-1 pl-1.5`, source map §6) — no test covers visual sizing, so this is invisible to CI, and the `dropdown` example (`site/examples/calendar/dropdown-menu.gsx`) is the only place to see it live.
- ADAPT (range semantics match upstream's `resetOnSelect: true`, not its default `addToRange`): react-day-picker's own default range behavior (`useRange.js`, source map §7.1) delegates every click to `addToRange`, which extends or replaces the existing `{from, to}` by date-arithmetic rules of its own. gsxui's `commitRange` (`ui/calendar.js`) instead implements the strict two-click machine that is upstream's `resetOnSelect: true` *opt-in* shape: no `from` set → set `from`; `from` set, no `to` → set `to`, swapping if the click precedes `from`; both already set → start over with a new `from`, discarding the old range entirely rather than extending it. `ui.Calendar` has no `resetOnSelect`/`excludeDisabled`/`min`/`max` parameters to switch this at the call site — there is exactly one range behavior, and it is upstream's non-default one.
- ADAPT (cell-level `range_start`/`range_middle`/`range_end` className slots are not ported): react-day-picker's own `classNames.range_start`/`range_middle`/`range_end` (`rounded-l-md bg-accent` / `rounded-none` / `rounded-r-md bg-accent`, source map §2) attach to the `<td>` cell. gsxui's DOM contract puts every range/selection data attribute on the `<button>`, not the cell (`data-range-start`/`data-range-middle`/`data-range-end`, already live via `calendarDayButtonClass`) — there is no cell-level `data-range-*` attribute to gate an equivalent cell class on, and adding one solely to reintroduce this slot would contradict the fixed DOM contract task 1 established. The button fills the cell edge-to-edge, so the range band stays visually continuous without it.
- PORTED (the `hidden` className slot, and `showOutsideDays` with it): an earlier revision of this ledger recorded `hidden` (`invisible`) as unportable because "no `data-hidden` attribute exists in gsxui's DOM contract", and recorded `ui.Calendar`'s `showOutsideDays` parameter as declared-but-read-nowhere. The final whole-branch review fixed both together, and the premise of the first turned out to be wrong about upstream too. react-day-picker 9.14.0's `helpers/createGetModifiers.js` does not remove a hidden day's cell: `showOutsideDays: false` sets `modifiers.hidden`, `DayPicker.js` still renders the `<td>` — carrying its own `data-hidden={modifiers.hidden || undefined}` — so the row keeps its seven columns, and only the DayButton inside is skipped (`!modifiers.hidden && isInteractive ? DayButton : …`); `classNames.hidden` is `invisible` rather than `hidden` precisely because the cell still occupies layout. That is the same shape as this port's fixed-42-cell grid, so `data-hidden` is now part of the DOM contract (bare presence on the cell, like `data-outside`/`data-today`/`data-disabled`), `calendarDayClass` carries `data-hidden:invisible`, and the resolved flag reaches the client as `data-gsxui-calendar-show-outside-days` so `calendar.js` can re-derive it for a month the server never rendered. The one thing gsxui cannot copy is skipping the button: `calendar.js` may never create or destroy an element, so the button is BLANKED instead — empty text, `tabindex="-1"`, `aria-hidden="true"`, native `disabled`. Hidden days are therefore OUT of the roving tab sequence, deliberately the opposite of the call made for DISABLED days (which stay in it): an invisible focusable button is a screen-reader trap, whereas a visible disabled one is a legitimate stop.
- ADAPT (`showOutsideDays` defaults to FALSE, where shadcn's prop defaults to true): shadcn's `calendar.tsx` declares `showOutsideDays = true` in its own parameter destructuring. gsxui has no way to give a `bool` component parameter a non-zero default — `|> default(…)` cannot distinguish "caller passed false" from "caller passed nothing" for a bool — so `<ui.Calendar/>` with the parameter omitted hides the padding days rather than showing them. Every shipped example passes it explicitly, and the parameter is not optional in spirit: it is one of the fourteen the design doc calls for. Recorded rather than papered over with an inverted `hideOutsideDays` parameter, which would have diverged from upstream's own name.
- MECHANISM (today is reconciled client-side): the server marks `data-today` from `time.Now().UTC()` — the server has no timezone of its own in this computation, it is pinned to UTC unconditionally, so server and client can disagree the instant a client isn't on the UTC calendar date (e.g. a client west of UTC, still on "yesterday" at 8pm local while UTC has already rolled over). On load, `reconcileToday` (`ui/calendar.js`) re-marks `data-today` from `clientToday()` — the client's own local calendar date, read via the local `Date` getters and normalized to UTC midnight for direct comparison against the UTC-midnight grid — touching only that one attribute on the 42 cells, not a full repaint. The client's local date wins; this is the one place server and client are expected to legitimately disagree, and the only attribute JS ever corrects independent of user interaction.
- MECHANISM (zero-value displayed month): `month.IsZero()` resolves before the grid reads it. Range mode prefers a non-zero `from`; otherwise the first `selected` date wins; otherwise the server's current UTC date wins. Selection-derived dates pass through `dayOnly`, so the month displayed on first paint is the same UTC calendar month used by selection comparison and serialization.
- MECHANISM (lossless multiple-mode form bridge): single mode renders one hidden input; range renders the `name` / `name+"-to"` pair; multiple renders one hidden input per selected ISO date, all with the caller's exact `name`, so native submission and `FormData.getAll(name)` carry every value. An unselected multiple calendar keeps one empty marked placeholder. `calendar.js` may create and remove only these `data-gsxui-calendar-hidden-multiple` controls as selection changes; the fixed-DOM rule protects the 42 day cells whose identity preserves focus, not unrelated form controls.
- MECHANISM (disabled roving stop remains reachable): disabled days outside the roving stop keep native `disabled`. A disabled day that is live-focused or owns the sole `tabindex="0"` uses `aria-disabled="true"` without native `disabled`, so Tab and arrow-key focus can enter or remain in the grid while click/Enter/Space still commit nothing. Hidden outside days remain natively disabled and untabbable.
- MECHANISM (form reset and reviewed mixed-form overlap): Calendar snapshots its server-rendered selection, then after native reset completes restores that selection, clears live/sticky focus state, synchronizes the hidden bridge, and navigates to the client's current month. Combobox also reflects its native bridge after reset. A form containing both therefore intentionally matches both modules' `reset:false` handlers; `jstest/support/selector-allowlist.ts` records that exact reviewed pair, and `calendar/form.gsx` plus the mixed reset browser regression prove the handlers restore disjoint descendants correctly.
- GAP: multi-month (`numberOfMonths`), week numbers (`showWeekNumber`), custom modifiers, and the footer (`props.footer`) are not ported. Multi-month, week numbers, custom modifiers and the footer are plain scope cuts with no equivalent mechanism at all yet, ledgered here rather than reproduced.
- ADAPT (locale text): `Calendar` takes `locale CalendarLocale` (wide and short month names, wide and short weekday names, `Caption` and `DayLabel` patterns over `{month}` `{year}` `{weekday}` `{day}`, and a ten-rune `Digits` set) in place of react-day-picker's `locale`/`formatters`. The zero value is English. The server writes the client-needed parts to `data-gsxui-calendar-locale` and `calendar.js` composes captions and day labels from that attribute, never from its own constants, so navigation text matches first paint by construction (`jstest/specs/calendar.spec.ts`, the localized agreement diff). The nav buttons' `aria-label`s are `i18n.T` messages (see `## internal messages`).
- Registry: `calendar.gsx` imports `ui/icon` (the nav chevrons) and `ui/native-select` (the dropdown caption's `<select>` pair) — `registry.Deps("calendar")` is `["i18n", "icon", "native-select"]`, pinned in `internal/registry/registry_test.go`. `HasJS("calendar")` is `true` (`ui/calendar.js`).
...
## combobox
The Base UI (`@base-ui/react`) filtered listbox (`registry/new-york-v4/ui/combobox.tsx`) — NOT the legacy Command+Popover "combobox" recipe some registries ship under the same word. Parts: `Combobox`/`ComboboxInput`/`ComboboxTrigger`/`ComboboxClear`/`ComboboxContent`/`ComboboxList`/`ComboboxItem`/`ComboboxGroup`/`ComboboxLabel`/`ComboboxEmpty`/`ComboboxSeparator`. Built on `ui/combobox.js`, which restates `ui/select.js`'s value model/form bridge/popover machinery for an input trigger and layers on a `command.js`-style focus-stays-in-the-input highlight cursor.
- FINDING (demo location, dead end avoided): `registry/new-york-v4/examples/combobox-*.tsx` are the OLD Command+Popover pattern (`Command`/`CommandInput`/`CommandItem` + `Popover`/`PopoverTrigger`/`PopoverContent`) and import nothing from `ui/combobox` — confirmed by `grep -rl "ui/combobox" registry/new-york-v4/examples/` returning zero hits. The real demos for this component live at `apps/v4/examples/base/combobox-*.tsx` (11 files), confirmed structurally identical to `registry/new-york-v4/ui/combobox.tsx` (same exports, same `data-slot`s) against `content/docs/components/base/combobox.mdx`. `basic`/`groups`/`clear` were ported as this port's own site examples, plus a `form` example demonstrating the hidden bridge; `multiple`/`custom`/`invalid`/`disabled`/`auto-highlight`/`popup`/`input-group`/`rtl` are out of this pass's scope (multi-select chips are a GAP below; the rest are prop-toggle variants not required by the task brief).
- GAP (`ComboboxValue` follows the deferred popup composition): shadcn's current source exports `ComboboxValue` for its button-triggered popup variant, but that variant requires a different anchoring and value-reflection mechanism from this port's shipped input-triggered composition. The earlier port exposed the inert span anyway and inserted it beneath Basic's input only to exercise the style contract, producing a disconnected "Choose a framework" label that the runtime never updated. `ComboboxValue` is not shipped until the popup composition itself is implemented and behavior-tested; Basic now matches shadcn's input-only composition.
- ADAPT (filter: `contains`, web-verified — NOT read from `@base-ui/react`'s own source): `@base-ui/react` is absent from this checkout, so unlike every other traced-from-`dist/` mechanism in this file, the filter algorithm couldn't be read from package source. It was instead resolved from Base UI's published docs and issue tracker (web-verified, not a source read): the default is `useFilter().contains`, an `Intl.Collator`-backed BOOLEAN match at `sensitivity: "base"` (case- and accent-insensitive), mirroring React Aria's own implementation. It returns a boolean, not a score — there is NO ranking and NO reordering, a real divergence from `## command`'s own `commandScore` fuzzy-ranking engine (a verbatim port of the `command-score` npm package). `ui/combobox.js` therefore only hides/shows items on every keystroke; it deliberately does NOT port `command.js`'s DOM-reordering pass.
- GAP (no shared JS with `ui/command.js`, `registry.Deps` reasoning): a JS `import` from `combobox.js` into `command.js` (or vice versa) would be invisible to `internal/registry.Deps`, which derives a component's dependency list by `go/parser` over the generated `ui/*.x.go` — Go source only. `command.js`'s `commandScore` ranking engine (and `combobox.js`'s own `contains` matcher) have no Go representation to scan, so an import edge between the two JS files would silently vanish from CLI vendoring: installing `combobox` would never pull `command.js`, or vice versa, even though the JS `import` graph said it should. `combobox.js` duplicates its own ~15-line matcher and highlight loop rather than import `command.js` at all — the same reasoning `## select`'s bespoke typeahead (not `command.js`'s scorer) already established.
- MECHANISM (open/close + form bridge, restated from `## select`): `ComboboxContent` rides `SelectContent`'s exact popover machinery — `popover="auto"` (top layer, light dismiss, free Esc), position:fixed anchoring (below the input's `InputGroup` here, rather than below a trigger button), and `data-state="open"` stamped synchronously BEFORE `showPopover()` (the flash-avoidance rule, `## animations`). The discrete-transition enter/exit class block is copied BYTE-IDENTICAL from `ui/select.gsx`'s own (`opacity-0 scale-95 transition-[opacity,scale,translate,display,overlay] transition-discrete duration-150 open:opacity-100 open:scale-100 starting:open:opacity-0 starting:open:scale-95` plus the four `data-[side=…]:starting:open:*translate-*-2` tokens). `Combobox`'s form bridge is a hidden `sr-only` `<input type="text" name value>` (not a `<select>` — a combobox's value is one string, not an enumerated option set) kept in sync by `combobox.js` on every commit/clear, dispatching a bubbling native `change` — interchangeable with `## select`'s own bridge from a consumer's point of view (`gsxui:select` on the root, `change` on the bridge).
- ADAPT (Content/List drop the `--anchor-width`/`--available-height` CSS vars): Base UI's Positioner primitive sets `--anchor-width`/`--available-width`/`--available-height`/`--transform-origin` custom properties at runtime for `ComboboxContent`/`ComboboxList` to consume (the raw source's own `w-(--anchor-width)`, `max-w-(--available-width)`, `origin-(--transform-origin)`, and `List`'s own `min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))` tokens). `combobox.js` implements fixed positioning instead (`## select`'s own `openContent` model, not a full anchor-positioning engine), so those vars are never set; the dependent tokens are dropped in favor of plain nova-retargeted values (`min-w-36 origin-top-left` on Content) — the same substitution `## select`'s own ADAPT entry documents for `SelectContent`'s `min-w-[8rem]→min-w-36`. **CORRECTED in review round 1**: List keeps the FIRST arm of its own `min()` — `max-h-[calc(--spacing(72)---spacing(9))]`, i.e. 252px — rather than collapsing to a flat `max-h-72` (288px), which a first draft did by mistake; the two numbers are NOT interchangeable (`calc(--spacing(72)---spacing(9))` is 18rem minus 2.25rem at the default `--spacing: 0.25rem`), and the flat value clipped the bottom ~40px of a full list against `ComboboxContent`'s own `max-h-72 overflow-hidden` with no way to scroll to it. nova's own one-off `duration-100` for this component is dropped the same way `## select`'s entry rejects it there: `duration-150` is the popover family's shared standard, supplied by the discrete-transition block above. `data-[chips=true]:...` (multi-select-only) is dropped as inapplicable — see the chips GAP below.
- MECHANISM (highlight cursor is NOT real DOM focus — `## command`'s model, NOT `## select`'s): items are permanently non-tab-stops (no `tabindex` at all); `combobox.js` never moves real focus onto an item. Instead it stamps `data-highlighted` on the current item and `aria-activedescendant` on the INPUT, exactly `command.js`'s own selection-tracking shape (focus never leaves the single text control) — a deliberate divergence from `## select`'s `SelectItem`, where real `.focus()` roves among items because there is no input to protect. `aria-selected` on `ComboboxItem` is a SEPARATE, simpler fact than `## select`'s own `isValue && isFocused` computation: it tracks the picked VALUE alone (driven directly by the `selected bool` param at render time, recomputed by `commit()` on every pick), matching `data-state="checked"|"unchecked"` — the two attributes carry the same information here (unlike Select's split), because Combobox's highlight cursor already has its own, separate attribute pair (`data-highlighted` + `aria-activedescendant`).
- MECHANISM (checkmark visibility, restated from `## select`): the indicator `<span>` is always server-rendered; visibility is CSS-gated via `group/combobox-item` + `hidden group-data-[state=checked]/combobox-item:flex`, standing in for Base UI's own mount-gated `ItemIndicator` the same way `## select`'s own entry documents for `SelectItem`'s `ItemIndicator`.
- ADAPT (`ComboboxInput`'s attrs split class vs. everything else): the source destructures `{className, ...props}` — `className` merges into `cn("w-auto", className)` on the wrapping `InputGroup`; every other extra prop spreads onto the input itself. gsx's single `attrs` bag doesn't destructure on its own, so `ComboboxInput` does it explicitly: `attrs.Class()` reaches `InputGroup` (aggregated with the `"w-auto"` default via the ordinary tailwind-merge precedence), `attrs.Without("class")` reaches the input — the identical wrapper/control split `## native-select`'s own `NativeSelect` doc comment documents. `InputGroupInput` keeps its OWN default `data-slot="input-group-control"` — **an earlier draft overrode it to `"input-group-input"`, a review round 1 CRITICAL finding**: `ui/input-group.gsx`'s `InputGroup` keys its focus ring off `has-[[data-slot=input-group-control]:focus-visible]`, so the override silently killed the only focus indicator the control had (a WCAG 2.4.7 failure) — fixed by dropping the override.
- MECHANISM (`ComboboxInput`'s addon inlines the trigger, but calls `ComboboxClear` directly — asymmetric, and deliberately so): the source polymorphically merges `ComboboxTrigger`'s props onto `InputGroupButton`'s own `<button>` via Base UI's `render` prop (Radix's `asChild` cousin) — one DOM button, two logical components layered on it. gsx has no clone-element merge, so nesting `ComboboxTrigger`'s own independently-real `<button>` inside another button-producing call would emit an invalid button-in-button; `ComboboxInput` builds that one button directly instead (`InputGroupButton` composed with `data-slot="combobox-trigger"` passed as an explicit non-parameter attribute — the same override mechanism `## input-group`'s own `InputGroupButton` doc comment documents for `data-size`). `ComboboxClear` has no such conflict and — **fixed in review round 1, see the finding below** — is now called directly: per the source map, `ComboboxClear` is ALWAYS an `InputGroupButton` composition (never an independent bare `<button>` the way `ComboboxTrigger`'s own standalone shape is), so calling it produces exactly one `<button>`, not two.
- FIX (`ComboboxClear`, review round 1, MINOR): a first draft gave the standalone `ComboboxClear` its own `[&_svg:not([class*='size-'])]:size-4` class and a hand-rolled `<button>` — both diverged from the source map (`cn(className)`, composing `InputGroupButton variant="ghost" size="icon-xs"`, "no base class of its own at all") and left a standalone `<ui.ComboboxClear/>` completely unstyled (no button visual treatment beyond the stray svg-size selector). Fixed to compose `ui.InputGroupButton` directly, matching source; `ComboboxInput`'s own clear addon now calls it instead of duplicating the composition inline.
- FIX (open-on-focus flashed the popover shut, review round 1, CRITICAL, user-reported and live-browser-traced): `combobox.js` used to open the popover on the input's `focus` event. `popover="auto"` light-dismisses on the pointer gesture ALREADY IN FLIGHT when focus fires mid-click (`pointerdown` → `focus` → the browser's own light-dismiss evaluation → `mouseup` → `click`, all one gesture) — opening from `focus` opens the popover mid-gesture, so completing that same gesture immediately closes it again: clicking the input flashed the list open and it vanished. The trigger chevron never had this bug because it opens on `click`, which lands AFTER light-dismiss has already been evaluated for that gesture. Fixed by opening the input the same way, on `click`, not `focus` — matching Base UI's own real behavior (opens on click/typing, not focus) and incidentally fixing Tab-through wrongly popping the list open on every combobox it passes, and Enter-on-the-chevron closing then instantly reopening (that reopen was `.focus()`, called after `hidePopover()` in the trigger's own click handler, re-entering the now-removed focus-opens-content handler).
- FIX (reopening filtered to the committed label, review round 1, CRITICAL): `openContent()` used to always run its filter pass against `input.value` — but `commit()` and `init()` both write the picked item's label INTO `input.value`. Reopening after picking "Remix" (`site/examples/combobox/basic.gsx`) left only "Remix" visible, every other option hidden; `site/examples/combobox/clear.gsx` hit the same bug on the FIRST open, since `init()` seeds "Go" from the server-rendered checked item. Fixed with an `input.dataset.gsxuiCommitted` flag: `commit()`/`init()`/the new `reflectFromBridge()` (see the reset FIX below) set it whenever they write a label into `input.value` programmatically; `openContent()` forces an EMPTY query (showing every option) while it's set; the `input` event handler clears it the moment the user actually types (typing was never affected — programmatic `.value` writes don't fire `input` events, so the flag only ever needs clearing on a REAL keystroke).
- GAP (review round 2, IMPORTANT — no server-side label derivation, fixed at the call site): the FIX above describes `init()` SEEDING the visible label from the server-rendered checked item, but that seeding is JS-only — before this round, `site/examples/combobox/clear.gsx` rendered `value="go"` with `selected={true}` on the checked item and NO `value` on `ComboboxInput` itself, so a no-JS request left the visible input completely empty (the hidden bridge still posted `"go"` correctly) and a JS request flashed placeholder-to-label on load. This violates the batch's own design rule (`docs/superpowers/specs/2026-07-24-tier4-batch-a-design.md` §4: a component takes its state as a server-rendered parameter and reflects it in the DOM) — `ComboboxInput` itself has no code path that derives a visible label from the selected `ComboboxItem`'s own children server-side; nothing computes "the checked item's text" at render time the way, say, `Sidebar`'s `data-state` is computed from `open`. Fixed at the call site, not in the component: `clear.gsx` now passes the picked item's own label through as `ComboboxInput`'s `value` attr directly (`attrs.Without("class")` already reaches the inner `<input>`, no signature change needed). `combobox.js`'s `init()` is kept as a fallback for callers who don't supply it, and now also stamps `gsxuiCommitted` when the input's value already matches the checked item's label (server-rendered case), not only when `init()` itself writes it — otherwise a server-reflected value with no `gsxuiCommitted` flag would retrigger this very FIX entry's bug on first reopen. The general gap remains: nothing in `ComboboxInput`/`Combobox` computes the label FOR a caller from `selected`/`value` alone — a caller with multiple selectable items must keep the input's `value` and the checked item's own label in sync by hand.
- FIX (`aria-activedescendant` survived on a closed combobox, review round 1, IMPORTANT): `clearValue()`'s own `filter()` call used to stamp `aria-activedescendant` unconditionally via `highlight()`, leaving the input pointing at an option inside a hidden popover while `aria-expanded="false"`. `highlight()` now checks `contentOf(root)?.matches(":popover-open")` and clears (never sets) `data-highlighted`/`aria-activedescendant` while closed, regardless of which item was asked for.
- FIX (mouse commit dropped focus, review round 1, IMPORTANT): `commit()` used to leave real DOM focus wherever the click landed (`document.body`, typically, since items are never tab stops) — subsequent typing/arrowing was dead. `commit()` now calls `input.focus()` after `hidePopover()`, the same as `ui/command.js`'s own `activate()` does for its selection.
- MECHANISM (`aria-haspopup`/`aria-controls` are now permanent, review round 1, IMPORTANT — APG expectation): both used to exist only while the popover was open (`aria-haspopup` wasn't rendered at all; `aria-controls` was set/removed by the `toggle` handler). `aria-haspopup="listbox"` is now server-rendered unconditionally on `ComboboxInput`'s own `<input>`; `aria-controls` is wired once, permanently, in `combobox.js`'s `init()` (pointing at `ComboboxList`'s id — not `ComboboxContent`'s; `role="listbox"` lives on `ComboboxList`), not toggled open/close.
- MECHANISM (form `reset` reflection, review round 1, IMPORTANT): a native `<form>.reset()` reverts the hidden bridge's `.value` to its content-attribute default (`Combobox`'s own server-rendered `value` param) automatically — browser-native, no code needed for the bridge itself. But the VISIBLE input and every item's `data-state`/`aria-selected` are separate elements the browser has no way to know about; without help they kept showing the last-picked label/state after a reset reverted the bridge underneath them. `combobox.js` now listens for `reset` on `form` (delegated, bubbles per spec) and re-derives the visible input text plus every item's checked state from the bridge's own (already-reset) `.value` via a new `reflectFromBridge()` helper — chosen over keeping a second copy of the value in the bridge's `value` ATTRIBUTE in parallel with the property, since this rides the browser's own native reset semantics instead of duplicating them.
- GAP (deferred, not built — multi-select and two React-only helpers): `ComboboxChips`/`ComboboxChip`/`ComboboxChipsInput`/`ChipRemove` (the multi-select chip-input composition) are not ported — this is a single-select-only port (`Combobox`'s own `value` param is one string). `ComboboxCollection` (a React items-array-to-JSX mapping helper) needs no gsx equivalent: a caller just writes an ordinary `for` loop over its own slice, calling `ComboboxItem` per element (`site/examples/combobox/groups.gsx` does exactly this for grouped data). `useComboboxAnchor` (`() => React.useRef<HTMLDivElement|null>(null)`, existing only so a multi-select caller can share one ref between `ComboboxChips` and `ComboboxContent`) has no gsx equivalent either — a gsx caller would pass an anchor selector string instead, if/when chips ship.
...
- MECHANISM (decided 2026-07-22, completed for the CSS-only style contract 2026-07-27): `asChild`/Slot is not ported and not needed for behavior attachment — the `data-gsxui-*` attributes are each interactive component's PUBLIC behavior contract, and fallthrough attrs deliver them to any element or component. A composed control also carries the logical trigger slot and its static ARIA attributes: `<ui.Button data-gsxui-dialog-trigger data-gsxui-slot-dialog-trigger aria-haspopup="dialog" aria-expanded="false">Open</ui.Button>`. Ordinary attribute forwarding keeps this caller marker alongside Button's own marker, so the output is one DOM button with `data-gsxui-slot-button data-gsxui-slot-dialog-trigger`: one element, both style roles, no cloning, wrapper, or slot helper. Document this idiom prominently per interactive component; only tag-swapping remains a (narrow, accepted) gap.
...
- GAP: Radix client context (trigger↔content wiring) replaced by closest("[data-gsxui-dialog]") proximity in JS. Local controls use that nearest-root ownership; an external control or one of several dialogs uses an authored content ID with native `commandfor`/`command="show-modal"` or `command="request-close"`.
- MECHANISM (public dialog commands): `gsxui:request-open` and `gsxui:request-close` are bubbling, cancelable `CustomEvent`s dispatched on the exact dialog. Their defaults run in a microtask after propagation, so late ancestor/document/window listeners may cancel them. Built-in reasons are `trigger`, `close-button`, `backdrop`, `cancel`, `shortcut`, and `select`; application reason strings are descriptive only and never select different behavior. `gsxui:open`/`gsxui:close` remain post-native-transition notifications.
- DOM-ONLY STATE/IDENTITY: `data-state` plus native `open` are the complete transition state — `closed` while `open` is still true means finite exit is in progress, and `open` cancels that pending close. A11Y IDs live on the DOM too: missing IDs use collision-checked `crypto.getRandomValues()` values, while authored IDs and ARIA relationships always win. `aria-haspopup` + initial `aria-expanded` are server-rendered on DialogTrigger; `aria-controls`, labels/descriptions, and expanded state are wired/synchronized by dialog.js.
- PROGRAMMATIC API: application code with a dialog reference uses the native `showModal()`, `show()`, `close()`, and `requestClose()` methods. gsxui exposes no imperative dialog helper; native lifecycle state and notifications reconcile through capture-delegated ToggleEvent handling (the native `close` event proved unreliable in current Chrome).
- ADAPT: request-driven exits stamp `data-state="closed"`, await only finite own animations, then call native `close()`; direct programmatic `dialog.close()` remains immediate by design.
...
- MECHANISM: `FieldSeparator`'s `data-content={!!children}` boolean stamp ports as the presence marker `data-content={children != nil}` — GSX omits it when false and emits it bare when true. `PaginationLink`'s `data-active={isActive}` uses the same binary-marker contract. The optional `field-separator-content` label span is gated by the same `children != nil` check via `{ if … }`.
...
## progress
- ADAPT: shadcn's `Progress` wraps Radix's `ProgressPrimitive.Root`/`ProgressPrimitive.Indicator` pair (`registry/new-york-v4/ui/progress.tsx`), driven by no client state of its own (a plain controlled `value` prop) but still routed through Radix's component machinery. This port replaces both with two plain `<div>`s and zero client JS — `role="progressbar"` plus `aria-valuemin="0"`/`aria-valuemax="100"`/`aria-valuenow={value}` (stamped directly, no `data-state`/`data-value`/`data-max` — nothing in shadcn's own class strings selects on those, so there is nothing for them to drive here) replace what Radix's `Root` stamps internally.
- FIX (theme-creator-parity Task 4, found reviewing the expanded preview by eye): both divs' class strings were meant to be carried token-for-token but weren't — the mechanical style porter only ever extracts the PER-STYLE presentational subset (`apps/v4/registry/styles/style-<name>.css`'s `bg-muted`/height/radius on the root, `bg-primary` on the indicator), never the STRUCTURAL classes the base component (`apps/v4/registry/bases/radix/ui/progress.tsx`) applies directly and identically in every style. The indicator in particular had no width or height set anywhere, in any style — `translateX`'s math was correct but had nothing visible to translate, so Progress silently rendered with zero fill in all 8 styles. `registry/canonical/progress.gsx` now carries those structural classes as a literal fragment alongside the recipe accessor (root: `relative flex w-full items-center overflow-x-hidden`; indicator: `size-full flex-1 transition-all`) — the same class-plus-recipe-accessor shape `Card`'s own root already uses for `group/card`.
...
- MECHANISM: the indicator's fill is Radix's `style={{ transform: translateX(-${100 - (value || 0)}%) }}` — ported as the identical `translateX` mechanism (not a `width` swap, which `transition-all` would animate differently). The declaration is written as a `css\`\`` literal rather than a Go concatenation: gw's CSS value filter blocklists `(`/`)`, and Go's `+` folds the static `translateX(…)` syntax and the dynamic percentage into one string the filter then judges as a unit, which is what previously forced `gsx.RawCSS` over the whole declaration. In a `css\`\`` literal the function call is static template text and only the percentage is a hole, so the filter still judges it and nothing is trusted that the component did not itself compute. The hole renders the `float64` directly, the same way `aria-valuenow` does.
...
- MECHANISM (discrete-transition reuse): the enter/exit class block (`opacity-0 scale-95 … transition-discrete duration-150 open:opacity-100 open:scale-100 starting:open:… data-[side=bottom]:starting:open:-translate-y-2`) ports byte-for-byte from `DropdownMenuContent`, replacing Radix's `data-[state=…]:animate-*`/`slide-in-from-*` keyframe block (same substitution ledgered under `## animations` and `## dropdown-menu`). Exit is inert for the same reason as dropdown's (popover is `display:none` before `data-state="closed"` is stamped) — closing snaps.
...
## dropdown-menu
- MECHANISM: hover highlight IS focus — shadcn item classes style `focus:` only because Radix moves roving focus onto hovered items; dropdown-menu.js ports that (pointerover → item.focus(); leaving the menu parks focus on the content via tabindex="-1" so arrow keys keep working).
...
- ADAPT: `dropdown-menu.js` sets `content.style.inset = "auto"` before setting `left`/`top` — the UA popover default rule (`inset: 0`) otherwise leaves `right: 0; bottom: 0` active alongside the hand-set `left`/`top`, collapsing/mispositioning the content since only two of the four inset edges were ever overridden.
...
- NOTE (superseded stopgap): positioning now runs through `gsxui.js`'s shared `position()` engine (below the trigger, left-aligned, 4px gap): main-axis flip with `data-side` tracking the placed side (a bottom-edge menu opens ABOVE and stamps `data-side="top"`, so the `data-[side=top]` enter slide is now live too; the authored `"bottom"` is restored on close), cross-axis shift, hard viewport clamp, reposition on window scroll/resize while open. Still not a full Floating-UI port — deltas in `ui/gsxui.js`'s positioning header. CSS anchor positioning remains the eventual native replacement.
- `DropdownMenuGroup`, `DropdownMenuCheckboxItem`, `DropdownMenuRadioGroup`/`DropdownMenuRadioItem`, and `DropdownMenuSub`/`DropdownMenuSubTrigger`/`DropdownMenuSubContent` ship (Tier 4 Batch B Task 1). Source: `docs/superpowers/plans/2026-07-25-tier4-source-map-menus.md` `## shared-items`.
- ADAPT: the `inset` prop on `DropdownMenuItem`/`DropdownMenuLabel`/`DropdownMenuSubTrigger` is a CSS-only axis — the caller stamps `data-inset` through attrs and each style's recipes carry `data-inset:pl-<N>` with per-style, per-family upstream values (lyra's menubar is pl-8 while its dropdown/context are pl-7). History: the prop was first dropped as dead weight, then revived as a components-layer escape-hatch rule in assets/css/styles/default/menu.css pinned to pl-7 — which was itself dead CSS (every recipe's own `px-*`/`pl-*` utility outranks components-layer padding, so nothing ever indented; found by the 2026-08-14 upstream parity review). The rule moved onto the recipes, where the `[data-inset]` variant outranks a caller's plain `pl-*` exactly as upstream's does.
- MECHANISM: `DropdownMenuItem` is a real menu item on `<div role="menuitem" tabindex="-1">` (Radix's own `Item` is also a non-native, ARIA-role div, so this isn't a native-first swap like checkbox/radio/switch — it's a straight structural port). `dropdown-menu.js`'s arrow-key roving focus walks `[role="menuitem"]:not([aria-disabled])` within the content; click on an item emits `gsxui:select` then closes via `hidePopover()`.
- A11Y: `aria-haspopup="menu"` and the initial `aria-expanded="false"` are server-rendered on `DropdownMenuTrigger`; `aria-expanded` is synced by `dropdown-menu.js` on the `toggle` event, same non-bubbling-event/capture pattern as dialog. On open, the first enabled menu item receives focus.
...
- ADAPT (verified in-browser): exit animations are inert — the native popover is display:none before data-state="closed" is stamped, so data-[state=closed]:animate-out never runs; closing snaps. Accepted for v1; an animated-close strategy (beforetoggle/allow-discrete or dialog-style cancelable request) is in the README backlog.
- MECHANISM (trigger click-close guard): with `popover="auto"`, a real pointerdown on the trigger is an *outside* pointerdown relative to the content (the trigger isn't inside it) — the popover API light-dismisses on that pointerdown before the subsequent click fires, so a bare `togglePopover()` in the click handler would see an already-closed popover and wrongly reopen it. `dropdown-menu.js` guards this by recording, on `pointerdown`, whether the content was open at that instant (`trigger.dataset.gsxuiWasOpen`); the `click` handler reads and clears that flag and no-ops if it was `"true"` (light dismiss already closed it), otherwise falls through to an explicit `hidePopover()`/`showPopover()` pair (never a bare `toggle`) so the keyboard-activation close path (no intervening pointerdown, flag stays `"false"`) still works. Real-pointer verification of the light-dismiss race is unavailable in this session's dev harness (no real pointer device to reproduce the pointerdown/click ordering); the guard is state-preserving either way — with or without the race actually firing, the trigger always converges on toggling from the popover's actual current state rather than an assumed one.
- ADAPT (submenus — popover nesting, not portalling, load-bearing): Radix portals `SubContent` to `document.body`, same as top-level `Content`; that has no equivalent on the native popover API. `DropdownMenuSubContent` renders `popover="auto"` DOM-nested inside its `DropdownMenuSub`, which itself sits inside the parent `DropdownMenuContent` (pinned, class strings included, by `TestDropdownMenuSubNestsContentInsideParentContentPinned` — a plain index-order check would be a tautology here, since the caller builds the tree and gsx has no portal to regress against). Measured in Chrome (both via `ui/dropdown-menu_test.go`-adjacent manual probing and a standalone nested-popover harness during this task):

  | scenario | parent's fate |
  |---|---|
  | child popover DOM-nested inside parent, opened with `showPopover()` | stays open |
  | child not nested, opened via a real `popovertarget` invoker click | stays open |
  | child not nested, opened programmatically with `showPopover()` | light-dismisses |
  | (this task, empirical) parent `hidePopover()`'d while a DOM-nested child is open | child auto-closes too, no manual cascade needed |

  A `popover="auto"` establishes its ancestor chain by DOM containment OR by invoker relationship; submenus open on `pointerenter` and `ArrowRight`, not only click, so every open is programmatic — DOM nesting is the only robust option. Also verified in-browser: the parent content's own `overflow-x-hidden overflow-y-auto` (plus a simulated `transform: scale()`, matching the enter/exit transition) does NOT clip the nested, open submenu — once shown, a popover is promoted to the top layer for painting/stacking, exempt from any DOM ancestor's overflow/clip/transform. No fix was needed on `DropdownMenuContent`'s own pinned class.
- MECHANISM (server-rendered checked state): `DropdownMenuCheckboxItem`/`DropdownMenuRadioItem` take a `checked bool` stamping `aria-checked` + `data-state="checked"|"unchecked"` on first paint — correct with JS disabled. Unlike Radix's own `ItemIndicator` (mounts/unmounts its check/circle icon by React state), the icon is always rendered server-side; visibility is pure CSS via the ancestor-selector arbitrary variant `[[data-state=checked]_&]:flex` on the indicator `<span>` (same `[[…]_&]` idiom as `sidebar.gsx`/`kbd.gsx`/`collapsible.gsx`) — chosen over a named `group/…:` marker specifically so the item's own pinned base class string needs no extra token. `dropdown-menu.js`'s click handlers only flip `aria-checked`/`data-state`; no icon markup is ever duplicated into JS (which would be an import invisible to `registry.Deps`).
- ADAPT (checkbox vs radio close behavior): selecting a checkbox item flips it in place and does NOT close the menu — a deliberate ADAPT per the task brief, not a Radix default (Radix's real `CheckboxItem` `onSelect` closes the menu unless the caller calls `preventDefault()`; this port simply never closes on a checkbox select, full stop). Selecting a radio item DOES close the menu, same as a plain `Item`. `data-gsxui-dropdown-radio-group` scopes the "clear every other item" sibling walk to one radio group, so a page with more than one group doesn't cross-clear.
- MECHANISM (select-from-within-a-submenu closes the WHOLE menu): a plain/radio item's click handler resolves the content to hide via `contentOf(item)` — walking up to the `[data-gsxui-dropdown]` root and back down to its OWN `[data-gsxui-dropdown-content]` — not `item.closest(CONTENT_SELECTOR)`, which from inside a submenu would resolve to the nearest `[data-gsxui-dropdown-sub-content]` ancestor and close only that submenu, leaving the root open with the sub-trigger still highlighted. One `hidePopover()` call on the root is sufficient: per this ledger's own measured cascade (the ADAPT above), closing an ancestor auto popover closes any nested-open descendant along with it.
- MECHANISM (fix round 1, 2026-07-25 — hook attributes MUST be component-namespaced, never a prefix shared across modules): `DropdownMenuCheckboxItem`/`RadioGroup`/`RadioItem`/`Sub`/`SubTrigger`/`SubContent` originally shipped with a shared `data-gsxui-menu-*` prefix (the plan's own original prescription, since reverted), on the theory that these six parts are byte-identical between `dropdown-menu.tsx` and `context-menu.tsx` (`## shared-items` §1) so their DOM hooks could be too. They cannot: `ui/gsxui.js`'s delegation registry (`on(type, selector, handler, opts)`) keys its handler map only by `` `${type}:${capture}` ``, and `dispatch` invokes **every** registered handler whose selector matches the event target, regardless of which module registered it. `ui/index.js` imports both `dropdown-menu.js` and `context-menu.js`, so an identical selector registered by both fires both modules' handlers on one event. Measured live on the built site (dropdown page, Tier 4 Batch B Task 2 review): one click on a checkbox item left `data-state`/`aria-checked` net-unchanged (handler A flips it, handler B immediately reads A's own write and flips it back) and fired `gsxui:change` twice; the same collision broke click-to-open on submenus (A opens and stamps `data-state="open"`, B sees it already open and closes it), made `ArrowUp`/`ArrowDown` inside a `SubContent` skip every other item (two competing keydown handlers both walking `ownItems`), and doubled `gsxui:open`/`gsxui:close` on every submenu toggle. Fixed by namespacing per component instead: `data-gsxui-dropdown-{checkbox-item,radio-item,radio-group,sub,sub-trigger,sub-content}` here, `data-gsxui-contextmenu-{checkbox-item,radio-item,radio-group,sub,sub-trigger,sub-content}` in `## context-menu` below — slotting into each component's own existing `data-gsxui-dropdown`/`data-gsxui-dropdown-content` and `data-gsxui-contextmenu`/`data-gsxui-contextmenu-content` namespaces. No other component in this codebase shares a hook prefix across two `.js` modules; this was the first, and the plan is amended to never prescribe one again (`## menubar`, Task 3, will need its own third namespace for the same shared-items parts).
...
- MECHANISM (submenu open/close): opens immediately on `pointerover`, `ArrowRight` (keyboard, moving focus into the submenu's first item explicitly — deliberately NOT relying on native popover focus-restoration on close, which would race this port's own hover-follows-pointer focus model), and click (idempotent, for touch/no-hover pointers); `ArrowLeft` inside a submenu closes it and returns focus to its trigger explicitly. Closing rides a pointer-leave grace period shaped exactly like `hover-card.js`'s own `scheduleShow`/`scheduleHide` timer pair (`SUB_CLOSE_DELAY = 100`, same value, per the task brief's "match the shipped hover-card.js grace-period precedent") — but unlike hover-card, only close is delayed; open is immediate, per the brief's own spec. Arrow-key-driven focus moves at a given level close any OTHER open sibling submenu at that level (a submenu opened earlier by hover, with the mouse then stationary, would otherwise stay visually open once keyboard focus moves elsewhere — mouse-driven moves don't need this, since actually leaving a sub-trigger by pointer movement already fires `pointerout`, scheduling the same close). `data-state="open"` is stamped on both the sub-trigger and its content BEFORE `showPopover()` — the same flash-avoidance rule as the top-level trigger's click handler.
- GAP (submenu scope, v1): submenu positioning prefers `data-side="right"` (open to the right of the trigger, top-aligned minus a 4px offset) through the same shared `position()` engine as the top-level content — a right-edge submenu flips to the left and stamps `data-side="left"`; main-axis flip only, not Floating-UI's full fallback-placement list. No `aria-controls` is wired from a sub-trigger to its content (would need a generated id, deferred — see `## select`'s own id-generation-at-init precedent for the eventual shape). No open-delay/hover-intent on submenu open (Radix's likely real behavior, `derived-not-read` per the source map) — only close has a grace period, per the brief's literal spec; revisit if diagonal mouse movement across sibling triggers proves to flicker in practice. No RTL/`dir` awareness (codebase-wide gap, not submenu-specific — see `## toggle-group`'s own GAP entry for this same limitation elsewhere).
...
- NOTE (superseded stopgap): positioning now runs through `gsxui.js`'s shared `position()` engine, centered above the trigger with a 6px offset — a top-edge tooltip flips below and stamps `data-side="bottom"` (authored `"top"` restored on close); shift/clamp and scroll/resize reposition come with it. Deltas from Radix's Floating-UI placement are in `ui/gsxui.js`'s positioning header.
...
## popover
- WIN: this is `dropdown-menu.gsx`/`dropdown-menu.js`'s own popover-API mechanism (native top layer replaces Portal, `popover="auto"` gives free light dismiss and Esc, `closest("[data-gsxui-popover]")` proximity wiring replaces Radix client context) with the menu semantics stripped: no `role="menu"`, no `role="menuitem"` items, no arrow-key roving focus, no click-on-item close. `PopoverContent` holds arbitrary content (a form, free text — shadcn's own demo is a dimensions form), not a list of selectable items, so none of `dropdown-menu.js`'s item-focused machinery has anything to attach to.
- ADAPT: alignment is centered, not left-aligned — Radix's own `Popover` default is `side="bottom" align="center"`, unlike `DropdownMenuContent`'s `align="start"`. `popover.js`'s positioning calc is `r.left + r.width / 2 - content.offsetWidth / 2` (trigger's horizontal midpoint minus half the content's own width) instead of dropdown-menu.js's bare `r.left`; `origin-top` (not dropdown's `origin-top-left`) replaces shadcn's Radix runtime transform-origin var (`--radix-popover-content-transform-origin`) for the same reason — the enter/exit scale always originates from top-center under a centered-below anchor.
- NOTE (superseded stopgap): placement now runs through `gsxui.js`'s shared `position()` engine — a reduced Radix-popper parity layer (main-axis flip with `data-side` tracking the placed side and the authored value restored on close, cross-axis shift, hard viewport clamp, reposition on window scroll/resize while open) rather than the earlier one-shot clamp-only anchor. Still not a full Floating-UI port; the deltas are documented in `ui/gsxui.js`'s positioning header. `data-side="bottom"` remains the server-rendered authored preference.
...
- MECHANISM (trigger click-close guard): identical to dropdown-menu.js's own — `popover="auto"` light-dismisses on the trigger's own outside `pointerdown` before the subsequent `click` fires, so `popover.js` records `trigger.dataset.gsxuiWasOpen` on `pointerdown` and the `click` handler reads/clears it, converging on the popover's actual current state rather than assuming it closed (see `## dropdown-menu`'s own MECHANISM entry for the full rationale).
...
- NOTE (superseded stopgap): placement now runs through `gsxui.js`'s shared `position()` engine (see `## popover`'s own NOTE): `data-side` is stamped with the PLACED side — `"top"` when a bottom-edge card flips — and restored to the authored `"bottom"` on close; the content shift-clamps against the viewport and repositions on scroll/resize while open.
...
- `ContextMenuGroup`, `ContextMenuCheckboxItem`, `ContextMenuRadioGroup`/`ContextMenuRadioItem`, and `ContextMenuSub`/`ContextMenuSubTrigger`/`ContextMenuSubContent` ship (Tier 4 Batch B Task 2). Source: `docs/superpowers/plans/2026-07-25-tier4-source-map-menus.md` `## shared-items`. `ContextMenuPortal` remains unported — no gsx equivalent (native popover top layer replaces Portal outright, same as every other popover-backed component in this codebase).
- MECHANISM (shared-items parity with `## dropdown-menu`): per the source map's own diff verdict, `Group`, `CheckboxItem`, `RadioGroup`, `RadioItem`, `Sub`, and `SubContent` are byte-identical between `dropdown-menu.tsx` and `context-menu.tsx`, modulo the component-name/CSS-var-name prefix — so `ContextMenuGroup`/`ContextMenuCheckboxItem`/`ContextMenuRadioGroup`/`ContextMenuRadioItem`/`ContextMenuSub`/`ContextMenuSubContent` port mechanically from their already-shipped `DropdownMenu*` counterparts: same nova-metrics ADAPT (`gap-1.5 rounded-md`, matching the item rows already shipped), same right-side check/dot indicator (`right-2`, `size-4`, following `ui/select.gsx`'s `SelectItem` precedent, `pr-8 pl-1.5` on the item row), same server-rendered `checked bool` → `aria-checked`/`data-state` MECHANISM (`## dropdown-menu`'s own doc comment, not re-derived here), same checkbox-does-not-close/radio-does-close ADAPT, same DOM-nested-not-portalled submenu ADAPT (`## dropdown-menu`'s own measured Chrome table applies verbatim — both components sit on the identical native-popover substrate), and the same root-not-nearest-content `hidePopover()` fix for "selecting an item inside a submenu closes the whole menu" (`contentOf(item)` walks to `[data-gsxui-contextmenu]`, never `item.closest(CONTENT_SELECTOR)`). The DOM hooks themselves are NOT shared with dropdown's own — `data-gsxui-contextmenu-checkbox-item`/`-radio-item`/`-radio-group`/`-sub`/`-sub-trigger`/`-sub-content` here vs. `data-gsxui-dropdown-*` there, each namespaced to its own component; see `## dropdown-menu`'s own MECHANISM entry (fix round 1) for why a shared `data-gsxui-menu-*` prefix across the two `.js` modules was tried first and had to be reverted.
- ADAPT (`ContextMenuSubTrigger` — the one real shared-items delta, source map ## shared-items §2): unlike the six parts above, `SubTrigger` is genuinely NOT byte-identical between the two shadcn sources — dropdown-menu's class carries `gap-2` (context-menu's lacks it) and dropdown-menu's trailing `ChevronRightIcon` carries an explicit `size-4` (context-menu's does not). nova's own `style-nova.css` harmonizes the gap to `gap-1.5` on BOTH `.cn-dropdown-menu-sub-trigger` and `.cn-context-menu-sub-trigger` (nova wins on visual disagreement, the adjudicated house rule), so `ContextMenuSubTrigger`'s class is `gap-1.5`, identical to `DropdownMenuSubTrigger`'s own pinned class. The missing icon `size-4` is preserved as a no-op divergence instead of copied from dropdown's version: the class's own `[&_svg:not([class*='size-'])]:size-4` descendant selector stamps `size-4` onto the `ChevronRightIcon` either way (verified: `icon.ChevronRight`'s own `svgIcon` wrapper defaults to `class="size-4"` regardless, so the caller-supplied class merges to `"size-4 ml-auto"` via tailwind-merge whether or not `size-4` is authored explicitly), so there is no visual difference to fix and no reason to deviate from context-menu's own upstream source here. NOTE: the two components' pinned render tests are therefore NOT byte-comparable strings on this one token — `DropdownMenuSubTrigger`'s pin merges to `class="ml-auto size-4"` (dropdown authors both tokens explicitly, in that order) while `ContextMenuSubTrigger`'s merges to `class="size-4 ml-auto"` (context-menu authors only `ml-auto`; tailwind-merge appends it after `svgIcon`'s own default `size-4`) — same rendered size, different literal class-attribute order, don't diff the two pins expecting equality.
- ADAPT: the `inset` prop on `ContextMenuItem`/`ContextMenuLabel`/`ContextMenuSubTrigger` is the same CSS-only `data-inset` attr axis as dropdown's (see `## dropdown-menu` for the full history including the dead escape-hatch rule) — each style's recipes carry `data-inset:pl-<N>` with per-style upstream values.
...
- MECHANISM: `ContextMenuTrigger` carries no `aria-haspopup`/`aria-expanded` of its own, unlike `DropdownMenuTrigger` — it is a passive AREA, not an interactive control (Radix's own `ContextMenuTrigger` renders a plain wrapper, not an ARIA-widget-shaped element either); there is no toggle-on-click affordance to describe to assistive tech the way a dropdown trigger button has. `context-menu.js`'s `toggle` handler correspondingly has no trigger to sync `aria-expanded` onto and no `gsxuiWasOpen` flag to clear — a straight subset of dropdown-menu.js's own toggle handler with the trigger-aria lines removed.
- Registry: `context-menu.gsx` now imports `ui/icon` (Tier 4 Batch B Task 2: `ContextMenuCheckboxItem`'s `Check`, `ContextMenuRadioItem`'s `Circle`, `ContextMenuSubTrigger`'s `ChevronRight`) — `registry.Deps("context-menu")` is `["icon"]`, same shape as `## dropdown-menu`'s own icon dependency. `HasJS("context-menu")` is `true` (`ui/context-menu.js`).

## menubar
- WIN (Tier 4 Batch B Task 3): this is `dropdown-menu.gsx`/`dropdown-menu.js`'s own popover-API mechanism, reused for every shared item type (`Item`, `CheckboxItem`, `RadioGroup`/`RadioItem`, `Sub`/`SubTrigger`/`SubContent`) exactly the way `## context-menu` already reused it — same DOM-nested-not-portalled submenu ADAPT, same server-rendered `checked bool` MECHANISM, same checkbox-does-not-close/radio-does-close split, same root-not-nearest-content `hidePopover()` fix for "selecting an item inside a submenu closes the whole menu" (`contentOf(item)` here walks to `[data-gsxui-menubar-menu]`, the per-menu pairing wrapper — NOT `[data-gsxui-menubar]`, the whole bar, and NOT `item.closest(CONTENT_SELECTOR)`, which would resolve to the nearest SUB-content from inside a submenu). Hooks are `data-gsxui-menubar-*` throughout, never a prefix shared with `dropdown-menu.js`'s own `data-gsxui-dropdown-*` or `context-menu.js`'s own `data-gsxui-contextmenu-*` — see `## dropdown-menu`'s own MECHANISM entry (fix round 1) for the incident this namespacing avoids repeating. Mechanically verified disjoint: a Node script imports every `ui/*.js` with `on()` shimmed to record every resolved `(type, capture, selector)` triple and asserts no triple is registered by more than one file — 116 registrations across 20 behavior modules, zero collisions (a deliberately-collided run against the same script, temporarily renaming a menubar selector to a dropdown one, was confirmed to fail loudly first, ruling out a false-negative checker).
- MECHANISM (`MenubarMenu` is the per-trigger root, `Menubar` is the bar): unlike `DropdownMenu`/`ContextMenu`, which each pair exactly one trigger with one content, a menubar holds N independent trigger/content pairs in one row. `MenubarMenu` (`data-gsxui-menubar-menu`, `class="contents"`) plays the SAME "root proximity scope" role `DropdownMenu`'s own root plays — `contentOf(trigger)` resolves via `closest("[data-gsxui-menubar-menu]")`, never the outer bar. `Menubar` itself (`data-gsxui-menubar`, `role="menubar"`) is a SEPARATE, wider scope: the roving-tabindex/open-follows-hover coordination surface across every `MenubarMenu`'s own trigger.
- MECHANISM (a. roving tabindex across triggers, `derived-not-read` — source map `## menubar` §2, no `@radix-ui/react-menubar` dist exists in the reference checkout to trace): follows `ui/toggle-group.js`'s own model exactly, restated rather than imported (no shared JS module — `registry.Deps` is derived by `go/parser` over the generated `.x.go`, so a JS-only shared module would be invisible to vendoring). `MenubarTrigger` server-renders NO `tabindex` attribute at all (graceful no-JS fallback: every trigger is its own tab stop until JS runs); a one-time init scan collapses each bar to exactly one `tabIndex=0` trigger (the first one, `toggle-group.js`'s own "pressed-or-first" priority collapses to just "first" here, since a trigger has no ARIA-pressed-shaped state to prefer), maintained by every arrow-key move and click afterward.
- MECHANISM (b. open-follows-hover, `derived-not-read` — source map `## menubar` §3, same source-absence caveat): while no menu in the bar is open, hovering a trigger does nothing — a menubar requires an explicit open (click/Enter/Space/ArrowDown) of the FIRST menu, unlike a plain dropdown/hover-card. Once any one menu in the bar is open, hovering a SIBLING trigger switches to its menu with no click (`isAnyOpen(bar)` gates this). `openMenu(trigger)` always closes any OTHER open content in the same bar before showing its own — sibling menus are never DOM-nested in one another (only a menu and its own submenus are, spec §1's nesting constraint applies there alone), so switching is a close-then-open, not a reparent.
- FIX ROUND 1 (arrow-switch-between-open-menus was dead code, corrected — not deleted): an earlier version wired "`ArrowLeft`/`ArrowRight` switch which menu is open" onto the TRIGGER's own `keydown` handler, gated on `isAnyOpen(bar)`. That branch was unreachable on the normal path: opening a menu moves focus onto its first ITEM (the `toggle` handler), so the very next arrow keydown targets that item, not the trigger — `closest("[data-gsxui-menubar-trigger]")` never matches an item, since the trigger is a sibling of the content, not an ancestor. Real fix: `ArrowLeft`/`ArrowRight` are now handled in the CONTENT-level `keydown` handler (`CONTENT_SELECTOR`, where keydowns while browsing an open menu's items actually land), gated on `!content.matches("[data-gsxui-menubar-sub-content]")` so a submenu's own `ArrowRight`-opens-a-further-submenu and `ArrowLeft`-closes-this-submenu meanings are unchanged — both resolve the owning trigger via `content.closest("[data-gsxui-menubar-menu]")`, then call a new shared `switchBarMenu(trigger, dir)` helper (steps to the adjacent ENABLED trigger, `setActiveTrigger`, `openMenu`). The trigger-level handler's own `ArrowLeft`/`ArrowRight` branch is KEPT, not deleted — it is still reachable for a menu with zero items (focus never leaves the trigger) and for a trigger refocused via Shift+Tab — but is now correctly understood as the secondary path, not the primary one; its own code comment was corrected to say so.
- RULING (`MenubarContent`'s missing `data-[state=closed]:animate-out`, source map `## menubar` §1 flag): new-york-v4's own `MenubarContent` class omits this one token from the otherwise-standard six-token `animate-in/out/fade/zoom` set every sibling Content/SubContent in this whole menu family carries, including menubar's own `MenubarSubContent` — corroborated as deliberate rather than a transcription slip, since nova's independently-authored `.cn-menubar-content` has the identical omission. This port's own established ADAPT (`## animations`, `popover.gsx`) already replaces that ENTIRE six-token tw-animate-css family, on every popover-based Content/SubContent in this codebase without exception, with one discrete-transition CSS block keyed off Tailwind's `open:`/`starting:open:` variants — a mechanism with no standalone "closed-state token" to selectively omit in the first place (the same `open:opacity-100`/base-`opacity-0` pair drives both directions). Ruling: `MenubarContent` gets the identical discrete-transition block every sibling Content/SubContent gets, same as `MenubarSubContent` — new-york-v4's own omission is recorded in `ui/menubar.gsx`'s own file header per the task's instruction to port what the source says, but has zero representation in this port's actual markup, since the token it omits was already replaced site-wide before this component existed.
- ADAPT (`MenubarSubTrigger` — two menubar-only spelling deltas, both flagged by the source map, both ruled on rather than silently ported or silently fixed): (1) `outline-hidden`, NOT new-york-v4's own `outline-none` — menubar.tsx's one-off spelling among the whole menu family (every other `Item`/`SubTrigger` class in this codebase, across all three menu components, already uses `outline-hidden`), and a real, if narrow, accessibility delta per the source map (`outline-none` removes the focus ring in forced-colors/high-contrast modes too; `outline-hidden` is Tailwind v4's own "hidden but still present for forced-colors" idiom) — ruled to follow this port's own existing house-wide convention over reproducing what reads as menubar.tsx's own copy-paste drift. (2) the trailing chevron is ported as `size-4`, not new-york-v4's own literal `h-4 w-4` (a third distinct 16px-icon spelling among the three menu families, alongside dropdown's `size-4` and context-menu's bare/unstyled one) — same no-op-divergence call already ledgered for `## context-menu`'s own `SubTrigger` icon, since the class's own `[&_svg:not([class*='size-'])]:size-4` descendant selector stamps `size-4` onto the icon either way.
- ADAPT (nova erases new-york-v4's own `rounded-xs` menubar delta on Item/CheckboxItem/RadioItem): new-york-v4's `MenubarCheckboxItem`/`MenubarRadioItem` carry `rounded-xs`, genuinely different from dropdown/context-menu's own `rounded-sm`. nova's own `style-nova.css` erases it: `.cn-menubar-checkbox-item`/`.cn-menubar-radio-item` land on the same `rounded-md` every item-shaped part in this whole menu family uses — the result is that `MenubarItem`/`MenubarCheckboxItem`/`MenubarRadioItem` end up BYTE-IDENTICAL, modulo the `data-slot`/`data-gsxui-*` prefix, to their already-shipped `DropdownMenuItem`/`CheckboxItem`/`RadioItem` pinned classes (see `ui/menubar_test.go`'s own pinned tests). `MenubarTrigger` is a separate exception that does NOT get bumped to `rounded-md` — nova's own `.cn-menubar-trigger` literally keeps `rounded-sm`, a genuine per-part nova value, not an oversight.
- FIX ROUND 1, CORRECTION (indicator side — an earlier version of this entry falsely implied menubar's CheckboxItem/RadioItem indicator geometry AGREES with nova; it does not): nova's own `.cn-menubar-checkbox-item`/`.cn-menubar-radio-item` are `py-1 pr-1.5 pl-7` (source map `## menubar` nova block), and `.cn-menubar-checkbox-item-indicator`/`-radio-item-indicator` are `left-1.5 size-4` — nova deliberately keeps MENUBAR's own indicator on the LEFT, unlike its own dropdown/context-menu rules (both `right-2`). This port does NOT follow nova here: `MenubarCheckboxItem`/`MenubarRadioItem` keep the RIGHT-edge geometry (`right-2`, `size-4`, `pr-8 pl-1.5` rows) — matching `DropdownMenuCheckboxItem`/`ContextMenuCheckboxItem` and `ui/select.gsx`'s `SelectItem` — a deliberate ADAPT for cross-menu-family visual consistency (this task's own brief called for it explicitly), overriding nova's own menubar-specific value rather than agreeing with it. The BYTE-IDENTICAL claim above is about the resulting CODE only (menubar's ported class matches dropdown's own), not about nova endorsing that geometry for menubar specifically.
- FIX ROUND 1, CORRECTION (`MenubarSubTrigger` is NOT in the byte-identical set — an earlier version of this entry wrongly included it): `MenubarSubTrigger` lacks `[&_svg]:pointer-events-none`, `[&_svg]:shrink-0`, and `[&_svg:not([class*='text-'])]:text-muted-foreground`, all present on `DropdownMenuSubTrigger`'s/`ContextMenuSubTrigger`'s own class — its chevron therefore renders in the default foreground color, not muted like its two siblings'. Checked whether nova's own `.cn-menubar-sub-trigger` specifies the muted-chevron triple: it does not — and neither does `.cn-dropdown-menu-sub-trigger`/`.cn-context-menu-sub-trigger` (re-verified against the source map's own nova blocks for all three), so nova is not the source of dropdown/context-menu's own three tokens either; those come from `dropdown-menu.tsx`/`context-menu.tsx`'s own new-york-v4 source, confirmed by re-reading the source map's own `## menubar` §1 quote of `menubar.tsx`'s `SubTrigger` class, which never carried them to begin with (menubar.tsx's own upstream omission, not a port-time slip). Nova being silent means the RULING is to keep the source-faithful omission (per this fix round's own instruction) rather than add the triple to match dropdown/context-menu — ported as new-york-v4's menubar.tsx actually has it, ledgered here as a real, deliberate divergence from its two siblings.
- ADAPT (`MenubarTrigger`'s own metrics, NOT the item-shaped nova pass): `rounded-sm px-1.5 py-[2px]` (nova) over new-york-v4's own `rounded-sm px-2 py-1`. `data-[state=open]:bg-accent`/`focus:bg-accent` are KEPT verbatim, NOT swapped for nova's own `hover:bg-muted`/`aria-expanded:bg-muted` rewrite — that pairing is a real interaction-model change (hover-driven, ARIA-attribute-keyed), out of a metric-tokens-only retarget's scope, and the standing house exception explicitly keeps `data-[state=…]:` selectors everywhere in this codebase.
- ADAPT (`Menubar` root metrics, a narrower ruling than `## navigation-menu`'s own bg-background drop — different task, cited for contrast, not as authority here): nova's `h-8 gap-0.5 rounded-lg p-[3px]` replace new-york-v4's own `h-9 gap-1 rounded-md p-1`; `border` is kept (house exception, not nova's `ring-1`). `shadow-xs` is DROPPED — nova's own `.cn-menubar` rule omits it entirely, the same SHADOW-PRESENCE category as the dozen-plus other components already retargeted to drop a shadow nova's own CSS doesn't carry (`## nova density`). `bg-background` is KEPT, even though nova's own `.cn-menubar` rule doesn't restate it either — ruled differently from `## navigation-menu`'s own `NavigationMenuTrigger`, which explicitly drops `bg-background` per that task's own independent finding ("a real, not just metric, style choice... don't silently reintroduce bg-background"): this task has no equivalent explicit finding for `Menubar`'s own root, and the master `## nova density` DECISION states colors are "explicitly out of scope and untouched" for a metric-tokens-only retarget — the conservative, defensible reading is to keep the color and treat nova's own omission as inconclusive rather than assume it's the same deliberate drop navigation-menu-trigger was independently ruled to be.
- MECHANISM (`MenubarContent`'s `overflow-hidden`, not dropdown/context-menu's own `overflow-x-hidden overflow-y-auto`): new-york-v4's own `MenubarContent` class carries no `max-h`/`overflow-y-auto` at all — it is not a scrollable clipped container, unlike `DropdownMenuContent`/`ContextMenuContent`. Per the source map's own `## shared-items` §5 note, this means the nested-submenu-clipped-by-a-scrolling-ancestor risk flagged there for dropdown/context-menu's own `SubContent` never arises for `MenubarSubContent` at all — recorded here as confirmation, not a new finding. `data-side="bottom"` is the server-rendered authored preference; menubar.js places through `gsxui.js`'s shared `position()` engine (see `## dropdown-menu`'s own NOTE), which stamps the placed side and restores the authored one on close.
- NOTE (min-width deltas, three different outcomes across three components — don't assume they're all the same swap): `MenubarContent`'s `min-w-36` is a GENUINE nova delta from new-york-v4's own `min-w-[12rem]` (12rem → 9rem, a real narrowing). `MenubarSubContent`'s `min-w-[8rem]` has NO delta — nova's own `min-w-32` for `.cn-menubar-sub-content` is exactly 8rem, the same value new-york-v4 already carries, so the original bracket spelling is kept rather than switching to nova's own token spelling for a value that didn't change. Contrast `## dropdown-menu`'s own `DropdownMenuSubContent`, whose nova value (`min-w-[96px]`, 6rem) DOES genuinely narrow from new-york-v4's shared `min-w-[8rem]` — three parts, three different outcomes, verified individually rather than assumed to pattern-match.
- ADAPT (`MenubarShortcut` keeps `ml-auto`, undisclosed until this fix round): nova's own `.cn-menubar-shortcut` is `text-muted-foreground group-focus/menubar-item:text-accent-foreground text-xs tracking-widest` — no `ml-auto` token, unlike `MenubarShortcut`'s ported class (`ml-auto text-xs tracking-widest text-muted-foreground`), which keeps it. `ml-auto` is not a metric token (it's the mechanism that pushes the shortcut to the row's right edge, not a size/spacing scale value), so it was never in scope for the metric-tokens-only nova retarget in the first place, and dropping it would break every Shortcut's own layout — every dropdown/context-menu `Shortcut` in this codebase keeps it too. nova's own `group-focus/menubar-item:text-accent-foreground` addition (a focus-driven color swap keyed off a `group/menubar-item` marker class) is also NOT ported — no such marker class exists anywhere on `MenubarItem`/`MenubarCheckboxItem`/etc. for it to key off, the same scope call every other Shortcut in this menu family already makes.
- FIX ROUND 1 (disabled triggers, roving tabindex/arrow-walk): `menubar.js`'s `normalize()` and trigger-level arrow-key walk originally used the FULL trigger list, unlike `ui/toggle-group.js`'s own precedent (`items.filter(i => !i.disabled)`) — a bar whose FIRST trigger was `disabled` got handed `tabIndex=0` on an unfocusable button, leaving the entire bar keyboard-unreachable; arrows could also land focus on a disabled trigger mid-bar. Fixed with a new `enabledTriggersOf(bar)` helper, used by `normalize()`, the trigger-level arrow walk, and the new `switchBarMenu` helper (both call sites above) — `setActiveTrigger` itself still iterates the FULL list (a disabled trigger still needs its own `tabindex="-1"` set explicitly). `TestMenubarTriggerDisabledRendersNativeAttr` (`ui/menubar_test.go`) covers the render contract this JS-side fix depends on (a real `<button disabled>`), the same "test what Go can test, at the render-contract layer" shape as `TestToggleGroupDisabledCascade`.
- MECHANISM (modifier-key bail, fix round 1): the trigger-level `keydown` handler now bails on any held modifier (`metaKey`/`ctrlKey`/`altKey`/`shiftKey`) before acting on `ArrowLeft`/`ArrowRight`, ported verbatim from `ui/toggle-group.js`'s own guard — without it, Cmd+ArrowRight (a browser/OS shortcut on a focused trigger) was being `preventDefault()`-ed and treated as a roving-tabindex move.
- Registry: `menubar.gsx` imports `ui/icon` (`MenubarCheckboxItem`'s `Check`, `MenubarRadioItem`'s `Circle`, `MenubarSubTrigger`'s `ChevronRight`) — `registry.Deps("menubar")` is `["icon"]`, same shape as `## dropdown-menu`'s/`## context-menu`'s own icon dependency. `HasJS("menubar")` is `true` (`ui/menubar.js`).

## navigation-menu
- SCOPE (Tier 4 Batch B Task 4, independent of Tasks 1–3): a hover mega-menu, not a menu of items — none of the `role="menu"`/`role="menuitem"` roving-focus machinery `## dropdown-menu`/`## context-menu`/`## menubar` share applies here. Modeled on `ui/hover-card.js`'s own hover/focus-open + grace-period-close precedent instead (the task brief's own explicit pointer) — every popover in this file is `popover="manual"` (top layer, no light dismiss/Esc; hover and focus alone drive open/close), not `dropdown-menu.js`'s own click+toggle-event model.
- GAP (shared-viewport mode not shipped, FIX ROUND 1 — supersedes an earlier version of this ledger entry that described a v1 architecture, reverted): shadcn's own `NavigationMenu` takes a `viewport` prop, defaulting true, which portals every `NavigationMenuContent` into one shared `NavigationMenuViewport` panel. This port ships ONLY the `viewport={false}` configuration — new-york-v4's own OTHER first-class supported mode, with its own real class strings (source map `## navigation-menu` §1's own `group-data-[viewport=false]/navigation-menu:` block), not an invented fallback. `NavigationMenuContent`'s chrome block keeps that `group-data-[viewport=false]/navigation-menu:` GATING STRUCTURE verbatim (FIX ROUND 2 — an intermediate version of this port stripped the prefix and applied the tokens unconditionally, reasoning that a gate which can never toggle is dead weight; reverted, since `NavigationMenu` now stamps `data-viewport="false"` by default, keeping the ancestor state the selector depends on present). The TOKENS behind that prefix are NOT byte-identical to upstream, by design — `rounded-lg` not `rounded-md`, `p-1` not `p-2 pr-2.5`, no `duration-200`/`duration-300` at all (superseded by the discrete-transition block's own `duration-150`, see below), and `w-full`/`md:w-auto` dropped from the unconditional first block entirely (the CRITICAL mobile-overflow fix, see below) — each its own ledgered ADAPT (FIX ROUND 3, correcting an earlier "character-identical"/"byte-identical" overstatement here and in `ui/navigation-menu.gsx`'s own doc comments and `ui/navigation-menu_test.go`, which is what "verbatim" above actually describes: the gate, not the whole class string). `NavigationMenuViewport` is DROPPED entirely, not kept as a no-op — the `viewport={false}` path this port ships never renders it in upstream shadcn either (`{viewport && <NavigationMenuViewport />}`), so there was nothing left for it to do; shipping it anyway would have repeated the "hook nothing binds to" mistake `## dropdown-menu`'s own history already caught once in this batch.
- NOTE (FIX ROUND 3 — `data-viewport="false"` is a DEFAULT, not unconditional): like every server-authored attribute gsx codegen emits, the stamp is wrapped in the standard `if !attrs.Has("data-viewport")` hatch — a caller passing their own `data-viewport` attr to `NavigationMenu` overrides it same as any other attribute would. Since `NavigationMenuContent`'s entire chrome block is gated on `group-data-[viewport=false]/navigation-menu:`, a caller-supplied `data-viewport` with any value other than `"false"` silently strips Content's border/bg/shadow/rounded chrome — not guarded against.
  - WHY (v1 measured broken, not merely suboptimal): the first version of this component built the shared viewport as a SEPARATE, second `popover="manual"` panel, shown/positioned/sized in lockstep with the active `Content` at the identical rect — "coincident, not containing." Measured live on the built site (transitions disabled to remove the frozen-clock artifact): `Content` was open with its own 3 links rendered; `Viewport` was ALSO open with `childElementCount: 0`; both were in the top layer, `Viewport` shown second so it painted ABOVE `Content`; `Viewport` carried an opaque background, a 1px border, and a 10px radius; `document.elementFromPoint` sampled at four points spread across the panel returned `navigation-menu-viewport` at every one — not a single sample landed inside `Content`; a screenshot confirmed a blank rounded panel with no links visible. Two coincident top-layer popovers cannot work when the upper one paints an opaque surface: it is a lid over the content, not a frame around it. The mega-menu rendered as an empty white box and every link inside was unreachable.
  - Two reasons a v2 attempt at the shared-panel mode is not revisited within this task: (1) gsx has no portal, and reproducing Radix's Content-portals-into-Viewport model without one means moving a live popover between DOM parents at runtime (a real option — `appendChild` genuinely moves a node without losing its listeners — deliberately not attempted here, since it needs reparenting bookkeeping this shape avoids entirely). (2) the source map's own decisive finding (`## navigation-menu` §3, unaffected by this fix) is that the eight nova-family stylesheets never TWEEN the viewport's own width/height — only a scale transform on open/close.
  - GAP (FIX ROUND 2, CORRECTION — an earlier version of this entry claimed the shared viewport "buys essentially nothing" this port doesn't already have; that overstated the finding above and is wrong, corrected here): "no tween" is not "no visible difference." What viewport={false} genuinely gives up, named rather than waved away: **(1) panel position** — viewport mode centers the ONE panel under the whole nav (shadcn's own outer wrapper: `absolute top-full left-0 isolate z-50 flex justify-center`); this port left-aligns each panel to its OWN trigger instead, so a panel opened from a right-hand trigger sits under that trigger, not centered under the bar — the largest visual delta of the two modes, and a real GAP, not a wash. **(2) one clipping/stacking box** — upstream's single `overflow-hidden` `isolate z-50` container makes switching between two triggers a crossfade INSIDE one frame; this port's two independent per-trigger popovers are a close-then-open of two separate top-layer elements instead, a visibly different transition shape even with matching per-panel fade/scale timing. **(3) in-place size change** — upstream's shared box still visibly SNAPS from one size to another on switch (the source map's own point: a snap, not a tween, but a snap is still a visible size change happening to one persistent box); this port has no equivalent at all — switching triggers here means one box (sized to its own content) closes and a differently-sized box opens elsewhere, not one box changing size. **(4) a sane small-viewport width** — upstream's `Viewport` itself carries `w-full` (the width of the nav, not the panel's own content), giving predictable mobile-width panels; this port's `Content` has no width utility of its own at all after the mobile-overflow fix below, so panel width is always shrink-to-fit-content, which can read narrower or differently-behaved than upstream's own small-viewport story. None of these four are reproduced by this port; they are the real cost of shipping `viewport={false}`, not an equivalent-minus-a-tween.
- FIX ROUND 2, CRITICAL (mobile-width overflow, found and fixed): `NavigationMenuContent`'s first class block carried new-york-v4's own `w-full`/`md:w-auto` pair verbatim, while `ui/navigation-menu.js` sets `position:fixed` on `Content` at EVERY breakpoint (not just below md the way upstream's own normal-flow-until-md-then-absolute layout does). For a fixed element the containing block is the viewport itself, not the `NavigationMenuItem` `<li>` — so below md, `w-full` resolved to ~100vw anchored at the trigger's own left edge, overflowing the right edge and forcing horizontal page scroll (measured live: a fixed `w-full` probe resolved to viewport width). Fixed by dropping `w-full`/`md:w-auto` outright — `Content` is shrink-to-fit at every breakpoint now (the same sizing `md:w-auto` already gave it above md), not by computing an explicit width in JS. Covered by `TestNavigationMenuContentHasNoFixedWidthUtility` (`ui/navigation-menu_test.go`).
- MECHANISM (positioning): `ui/navigation-menu.js` positions each `Content` under ITS OWN trigger's `getBoundingClientRect()`, matching new-york-v4's own `viewport={false}` anchoring, which targets the trigger's own `NavigationMenuItem` (`class="relative"`), not a shared panel location — the same hand-rolled-fixed-position stopgap every sibling popover in this codebase uses (`## dropdown-menu`'s own NOTE); `position:fixed` overrides the authored `top-0`/`left-0`/`top-full` tokens regardless, since a popover promoted to the top layer is not positioned relative to its DOM ancestor even when authored `absolute`. FIX ROUND 2: the JS-side offset dropped from 6 to 0 — the class's own `mt-1.5` margin (a margin still offsets a fixed-positioned box's own top edge from its computed `top`) already supplied the 6px trigger-to-panel gap; stacking a `+6` JS offset on top of it doubled the gap to 12px.
- GAP: the six-token `data-[motion=…]`/`animate-in`/`animate-out`/`fade-in`/`fade-out` direction-aware slide on `NavigationMenuContent` (new-york-v4's own "which side did this panel enter from" animation, driven by Radix's own runtime index tracking, `derived-not-read`) is not reproduced. Per the task's own binding decision ("the popover family's discrete-transition block is reused byte-identically"), `Content` gets the identical discrete-transition block every other popover in this codebase carries (`## animations`) instead — the same six-tokens-replaced-by-one-mechanism RULING `## menubar`'s own `MenubarContent` entry already made for its own (differently-shaped) omission. new-york-v4's own `duration-200` (nova: `duration-300`), which timed that six-token set, is not silently dropped — it has no home in the replacement mechanism (the discrete-transition block carries its OWN `duration-150`, byte-identical across every popover in this codebase per the task's own binding decision), so it is SUPERSEDED, not omitted (FIX ROUND 2 — an earlier version of this port dropped the token with no ledger line at all). Reusing the discrete-transition block requires `Content` to genuinely be `popover`-backed (the `open:`/`starting:open:` Tailwind variants only match `:popover-open`), which is itself the reason `Content` is independently a popover rather than a plain toggled `<div>`.
- ADAPT: `data-side="bottom"` is server-rendered statically, the same no-runtime-side-tracking substitution as every sibling popover's own ADAPT (`## dropdown-menu`'s own NOTE) — `ui/navigation-menu.js` always opens below the trigger.
- ADAPT (nova, `NavigationMenuTriggerStyle`/`NavigationMenuTrigger`, FIX ROUND 2 — the source map's own per-part "metric deltas worth naming" lists are illustrative, not exhaustive, corrected here the same way `## menubar`'s own `MenubarTrigger` ADAPT already established when it adopted `rounded-sm`/`px-1.5`/`py-[2px]` despite that list omitting it; this file previously under-adopted the equivalent tokens for the identical reason): `rounded-md` → `rounded-lg`, `px-4 py-2` → `px-2.5 py-1.5`, `transition-[color,box-shadow]` → `transition-all`, all straight from `.cn-navigation-menu-trigger`. `bg-background` is dropped entirely — nova's own rule carries no background-color utility at all, a real, not just metric, style choice per the source map's own explicit finding ("port nova's version as written… don't silently reintroduce `bg-background`"). Nova's FURTHER hover-model rewrite (`hover:bg-muted`/`focus:bg-muted`, `data-open:` shorthand replacing `data-[state=open]:`, `disabled:opacity-50` alone replacing `disabled:pointer-events-none disabled:opacity-50`) is NOT adopted — a real interaction-model/color change, out of scope the same way `## menubar`'s own `MenubarTrigger` ADAPT already ruled for the identical situation; the standing house exception keeps `data-[state=…]:` everywhere in this codebase, and colors/interaction are untouched by a metric-tokens-only nova retarget except where a source-map finding explicitly says otherwise (as it does, narrowly, for `bg-background` alone). The chevron keeps new-york-v4's own `group-data-[state=open]:` selector, not nova's own `group-data-open:`/`group-data-popup-open:` pair — the second of which belongs to Base UI's differently-shaped primitive per the source map's own provenance note and has no Radix `data-state` equivalent to key off here. A literal space text node between `{children}` and the chevron (FIX ROUND 2, previously missing) matches new-york-v4's own `{children}{" "}` (`navigation-menu.tsx:76`) byte-for-byte, even though the icon's own `ml-1` already supplies the visual gap either way.
- ADAPT (nova, `NavigationMenuList`, FIX ROUND 2): `gap-0` replaces new-york-v4's own `gap-1`, straight from `.cn-navigation-menu-list` — previously omitted, same "illustrative not exhaustive" correction as the Trigger's own.
- ADAPT (nova, `NavigationMenuContent`, FIX ROUND 2): `p-1` replaces new-york-v4's own `p-2 pr-2.5`, straight from `.cn-navigation-menu-content` — previously omitted, same "illustrative not exhaustive" correction as the Trigger's own.
- ADAPT (nova, `NavigationMenuLink`): `rounded-sm` → `rounded-lg`, WITH nova's own `in-data-[slot=navigation-menu-content]:rounded-md` override preserved (a real, if narrow, per-context nova value, the same `in-data-[slot=…]` idiom `button.gsx`'s own `in-data-[slot=button-group]:` rounding bump already uses) — new-york-v4's own `flex-col gap-1` structure (a mega-menu tile: title stacked over description) is kept over nova's own `flex items-center gap-2` rewrite, read as a plain single-line list-item shape for a differently-composed nav-menu variant, not a metric bump. Colors stay on accent, not nova's own muted rewrite — same out-of-scope ruling as the Trigger's own.
- ADAPT (`NavigationMenuIndicator`): the `data-[state=hidden|visible]:animate-out/in fade-out/in` pair is replaced with a plain CSS `opacity` transition, not the popover-family discrete-transition block — `Indicator` is not popover-attached (no `:popover-open` state to key `open:` off), so the byte-identical block `Content` reuses does not apply here. Positioning (`translateX` off the active trigger's own rect, relative to `NavigationMenuList`) is computed by `ui/navigation-menu.js`, which also injects `position:relative` onto the list at the same time — neither shadcn's own class nor nova's carries a position token for either element (Radix's own runtime sets it inline), the same "computed position via JS, not baked into the class" idiom every fixed-positioned popover in this codebase already uses for its own `position:fixed`. `NavigationMenuIndicator` survives the shared-viewport GAP above because it is mode-independent upstream too — it has no `group-data-[viewport=false]` selectors of its own to begin with, unlike `Content`.
- ADAPT (`NavigationMenuIndicator`'s `pointer-events-none`, FIX ROUND 3, not upstream's own token): unlike Radix's own `Indicator`, which unmounts via Presence when hidden, this port keeps it permanently in the layout — `data-state` alone toggles `opacity` (the ADAPT above) — so once `ui/navigation-menu.js` absolutizes it under the list there would otherwise be a real, hit-testable strip sitting just below the trigger bar even at `opacity-0`. Added since the part is decorative in every state and should never intercept a pointer event. No example composes `Indicator` yet (none of the two shipped examples places one inside their own `NavigationMenuList`), so this part has not been browser-exercised end-to-end — considered and deliberately not added, since a bare, purposeless `<ui.NavigationMenuIndicator/>` in an example would be contrived rather than illustrative.
- MECHANISM (hover grace period + open-follows-hover once one panel is open): `OPEN_DELAY`/`CLOSE_DELAY` (100ms each) reuse `ui/hover-card.js`'s own values — Radix's own real `delayDuration` is `derived-not-read` (no dist in the reference checkout), so the house default for a hover-delay component is used instead, per the brief's own "match the shipped hover-card.js precedent" instruction. Opening the FIRST panel in a menu rides that grace period (avoiding an accidental trigger while the pointer merely passes over the bar); switching between two ALREADY-open triggers in the same menu is immediate, no delay — the same "connected bar" feel `## menubar`'s own open-follows-hover already established for its sibling-trigger switch. Reposition on scroll/resize while open comes from `gsxui.js`'s shared `position()` engine, the same as `dropdown-menu.js`'s/`hover-card.js`'s own. No Escape/outside-pointerdown light dismiss — same deliberate choice as `## hover-card`'s own (hover/focus drive it, not outside clicks or Esc).
- Mechanically verified disjoint (re-run after FIX ROUND 2's changes): the same `on()`-shimmed Node-script check `## menubar`'s own ledger entry describes (extended to cover `ui/navigation-menu.js`) found 126 registrations across 21 behavior modules, zero collisions — validated by deliberately colliding one `navigation-menu.js` selector with an existing `menubar.js` one, confirming the checker fails loudly, then reverting.
- Registry: `navigation-menu.gsx` imports `ui/icon` (`NavigationMenuTrigger`'s `ChevronDown`) — `registry.Deps("navigation-menu")` is `["icon"]`, same shape as `## dropdown-menu`'s/`## context-menu`'s/`## menubar`'s own icon dependency. `HasJS("navigation-menu")` is `true` (`ui/navigation-menu.js`).
...
- STACKING/TIMERS (fresh design, no source to port): a plain array of toast
  ...
  `transitionend`, capped at `600ms` — deliberately unlike dialog.js's
  finite-own-animation completion, because this cloned-node transition needs a
...
- LEDGER (review round 2, MINOR — inverted bool default, accepted): `##
  dialog`'s own CONVENTION entry says "name parameters so the zero value is
  the shadcn default" (bools invert so Go's zero value, `false`, matches
  upstream's own default). `SidebarProvider(open bool, …)`/`Sidebar(open
  bool, …)` break that convention — shadcn's own `defaultOpen` is `true`
  (an uncontrolled sidebar starts EXPANDED), so the Go zero value (`open ==
  false`) renders COLLAPSED, the opposite of upstream's default. This was
  not caught by the convention because `open` isn't a new parameter with a
  free naming choice: review round 1's own CRITICAL fix (see this section's
  first entry) added it specifically to match the value the caller ALREADY
  passes to `SidebarProvider` one level up — inverting it to `collapsed
  bool` here would desync the two call sites' own params (a caller would
  pass `open` to one and `!collapsed` to the other) for zero-value-parity
  gain. Every shipped example passes `open` explicitly (no example relies
  on the zero value), so the divergence is silent for all of this port's
  own code; it would only surface for a NEW caller who calls
  `SidebarProvider(false, …)`/`Sidebar(false, …)`-by-omission expecting
...
## internal messages (`i18n.T`)

- RULE: any English string a component writes on an element or text node that a caller cannot reach through children, props or `attrs` is written as `i18n.T("...")`, with the English text as the message id. Caller text never passes through `T`. `ui/i18n_test.go` enforces both halves: `TestSweptMessagesOnlyAppearInsideT` for the swept table and `TestOverridableLiteralsPrecedeAttrs` for literals that stay plain because `{ attrs... }` follows them.
- MECHANISM: `i18n.T` is `type T string` in `ui/i18n/i18n.go`, a leaf directory package vendored like `ui/icon`. It is a separate package because gsx compiles a registered renderer in as an import from every generated component file, and the translator must name `T`, so a `T` in package `ui` would be an import cycle. It renders as written until the consuming module registers a renderer in its `gsx.toml` (`"<module>/<ui>/i18n.T" = "<module>/<ui>/i18n.Translate"`), with the translator beside the type, per gsx's translation-keys pattern. Renderers bind at `gsx generate` in the consuming module, so translation reaches vendored components only; a direct import of `github.com/gsxhq/gsxui/ui` uses the committed `.x.go` and renders English. No context value, no label props.
- Registry: `registry.Helpers()` lists it; `Components()` does not. Every component that writes `i18n.T(...)` depends on `i18n`, derived from the `ui/i18n` import like `ui/icon`, so `gsxui add <component>` vendors it.
...
- ADAPT (side stays physical in full): upstream skips only positioning prefixes inside `data-[side=left|right]` variants and still maps `border-r` to `border-e`. gsxui leaves every token in a side-keyed arm or variant untouched (`switch side` arms in Sheet, `switch direction` arms in Drawer, `[data-side=…]` selectors in Sidebar, and Drawer's `data-[vaul-drawer-direction=…]` variants), per the physical-side ruling `jstest/specs/rtl.spec.ts` pins.
