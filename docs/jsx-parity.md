...
- NOTE: positioning is a hand-rolled `position: fixed` anchor to the trigger's `getBoundingClientRect()` in `dropdown-menu.js` (open below, left-aligned, 4px gap), not Radix's Floating-UI collision-aware placement. CSS anchor positioning (`anchor()`/`position-anchor`) — the eventual native replacement — is not yet Baseline (Chrome-only as of this writing); JS positioning is a stopgap until it is. `data-side="bottom"` is stamped statically (dropdown-menu.js always anchors below), so the ported `data-[side=bottom]:slide-in-from-top-2` enter slide is live; the other three `data-[side=*]` selectors are dead until real placement logic lands.
...
- MECHANISM (`MenubarContent`'s `overflow-hidden`, not dropdown/context-menu's own `overflow-x-hidden overflow-y-auto`): new-york-v4's own `MenubarContent` class carries no `max-h`/`overflow-y-auto` at all — it is not a scrollable clipped container, unlike `DropdownMenuContent`/`ContextMenuContent`. Per the source map's own `## shared-items` §5 note, this means the nested-submenu-clipped-by-a-scrolling-ancestor risk flagged there for dropdown/context-menu's own `SubContent` never arises for `MenubarSubContent` at all — recorded here as confirmation, not a new finding. `data-side="bottom"` is server-rendered statically (menubar.js always anchors below the trigger), same hand-rolled-fixed-position stopgap as dropdown's own NOTE.
