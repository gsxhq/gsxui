...
- GAP: `ContextMenuCheckboxItem`, `ContextMenuRadioGroup`/`ContextMenuRadioItem`, `ContextMenuSub`/`ContextMenuSubTrigger`/`ContextMenuSubContent`, `ContextMenuGroup`, and `ContextMenuPortal` are not ported — post-v1-backlog scoping, tracked as its own Tier 4 Batch B task (`## dropdown`'s own equivalent parts — `DropdownMenuCheckboxItem`/`DropdownMenuRadioGroup`&`RadioItem`/`DropdownMenuSub`-family/`DropdownMenuGroup` — have since shipped there; this entry is context-menu's own remaining gap, not analogous to a still-open one). Only `ContextMenu`, `ContextMenuTrigger`, `ContextMenuContent`, `ContextMenuItem`, `ContextMenuLabel`, `ContextMenuSeparator`, `ContextMenuShortcut` ship.
