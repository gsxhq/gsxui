...
- MECHANISM (viewport sizing, source map `## navigation-menu` §2/§3, decisive): the two Radix runtime CSS custom properties that drive `Viewport`'s own sizing (`--radix-navigation-menu-viewport-width/height`) are set by `@radix-ui/react-navigation-menu`'s own `Viewport` internals, absent from the reference checkout — `derived-not-read` for the mechanism itself. What IS directly verifiable (grepped across all eight nova-family stylesheets' own `.cn-navigation-menu-viewport` rules): no CSS anywhere transitions `width`/`height` on the viewport, only a `scale` transform gated on open/close — so no per-frame tween is needed to match the visual contract. `ui/navigation-menu.js` sizes the viewport with one `getBoundingClientRect` read off the newly-active `Content` on activation, plus a `ResizeObserver` on that same `Content` for late-arriving resizes (an image finishing loading, dynamically populated content) — explicitly not a `requestAnimationFrame` tweening loop, which the reference does not have.
- ADAPT (ARCHITECTURE, load-bearing — no portal, and no DOM-node-moving either, a genuinely new shape among this codebase's menu family): Radix's own `NavigationMenuContent`, when a shared `Viewport` exists, portals into it so only the active panel's real DOM lives inside the one bordered/shadowed box, which is how `Viewport` gets to measure and resize itself off whatever child happens to be mounted. gsx has no portal, and this port does not simulate one by reparenting DOM nodes at runtime either (a real option — `appendChild` genuinely moves a node without losing its listeners — but rejected here for the added reparenting bookkeeping it would need, in favor of a shape needing none). Instead every `NavigationMenuContent` renders DOM-nested inside its own `NavigationMenuItem` (never moved), matching the DOM-nesting-not-portalling ADAPT every sibling submenu in this codebase already uses (`## dropdown`'s own SUBMENUS paragraph), and is independently `popover="manual"`, escaping overflow/clipping via the native top layer exactly like every other popover here. `NavigationMenuViewport`, when the ancestor `NavigationMenu`'s own `data-viewport!="false"`, is a SEPARATE, second `popover="manual"` panel shown/positioned/sized in lockstep with whichever `Content` is currently active — coincident, not containing: both open at the identical fixed `left`/`top` rect (computed off the `List`'s own `getBoundingClientRect`, so every item's panel appears at the same on-screen spot — the actual "shared viewport" visual contract), `Viewport` supplies the chrome (border/bg/shadow) and is resized to the active `Content`'s own measured width/height, `Content` itself stays unchromed in this mode (its own `group-data-[viewport=false]/navigation-menu:` chrome block simply never matches, same as upstream — `data-viewport="false"` mode needs zero extra JS at all, since `Content` is then `md:absolute` relative to its own `relative` `NavigationMenuItem` and toggles purely on the `:popover-open` state it already carries).
- GAP: the six-token `data-[motion=…]`/`animate-in`/`animate-out`/`fade-in`/`fade-out` direction-aware slide on `NavigationMenuContent` (new-york-v4's own "which side did this panel enter from" animation, driven by Radix's own runtime index tracking, `derived-not-read`) is not reproduced. Per the task's own binding decision ("the popover family's discrete-transition block is reused byte-identically"), `Content` and `Viewport` both get the identical discrete-transition block every other popover in this codebase carries (`## animations`) instead — the same six-tokens-replaced-by-one-mechanism RULING `## menubar`'s own `MenubarContent` entry already made for its own (differently-shaped) omission. Reusing that block requires `Content` to genuinely be `popover`-backed (the `open:`/`starting:open:` Tailwind variants only match `:popover-open`), which is itself the reason `Content` is independently a popover rather than a plain toggled `<div>` (see the ADAPT above).
- ADAPT: `origin-top`/static `data-side="bottom"` replace Radix's own runtime transform-origin var and dynamic side-tracking, the same no-runtime-var-to-read substitution as every sibling popover's own ADAPT (`## dropdown`'s own NOTE) — `ui/navigation-menu.js` always opens below the list.
- ADAPT: `h-[var(--radix-navigation-menu-viewport-height)]`/`md:w-[var(--radix-navigation-menu-viewport-width)]` are replaced with direct inline `width`/`height` set by `ui/navigation-menu.js` — the source map's own explicitly-offered alternative to a custom property (`## navigation-menu` §3), chosen since no Radix runtime exists in this checkout to set those two variables in the first place.
...
- Mechanically verified disjoint: the same `on()`-shimmed Node-script check `## menubar`'s own ledger entry describes (extended to cover `ui/navigation-menu.js`) found 126 registrations across 21 behavior modules, zero collisions — validated by deliberately colliding one `navigation-menu.js` selector with an existing `dropdown.js` one, confirming the checker fails loudly, then reverting.
