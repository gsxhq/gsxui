...
Current gsxui components emit token-membership
`data-gsxui-slot="<token> [<composed-token>...]"` hooks and no presentation
classes. `assets/css/foundation.css` owns behavior mechanics,
...
- MECHANISM (decided 2026-07-22, completed for the CSS-only style contract 2026-07-27): `asChild`/Slot is not ported and not needed for behavior attachment — the `data-gsxui-*` attributes are each interactive component's PUBLIC behavior contract, and fallthrough attrs deliver them to any element or component. A composed control also carries the logical trigger slot and its static ARIA attributes: `<ui.Button data-gsxui-dialog-trigger data-gsxui-slot="dialog-trigger" aria-haspopup="dialog" aria-expanded="false">Open</ui.Button>`. `withSlot` merges this caller token with Button's own slot, so the output is one DOM button with `data-gsxui-slot="button dialog-trigger"`: one element, both style roles, no cloning or wrapper. Document this idiom prominently per interactive component; only tag-swapping remains a (narrow, accepted) gap.
