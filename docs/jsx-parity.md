...
- ADAPT: the `inset` prop on `DropdownMenuItem`/`DropdownMenuLabel`/`DropdownMenuSubTrigger` is dropped along with it — `data-[inset]:pl-8` is removed from all three classes as dead weight, the same "drop the selector, don't ship dead CSS" call as dialog's close-button ADAPT and avatar's size ADAPT.
