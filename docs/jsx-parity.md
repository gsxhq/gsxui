...
- Registry: `context-menu.gsx` now imports `ui/icon` (Tier 4 Batch B Task 2: `ContextMenuCheckboxItem`'s `Check`, `ContextMenuRadioItem`'s `Circle`, `ContextMenuSubTrigger`'s `ChevronRight`) — `registry.Deps("context-menu")` is `["icon"]`, same shape as `## dropdown`'s own icon dependency. `HasJS("context-menu")` is `true` (`ui/context-menu.js`).

## menubar
- WIN (Tier 4 Batch B Task 3): this is `dropdown.gsx`/`dropdown.js`'s own popover-API mechanism, reused for every shared item type (`Item`, `CheckboxItem`, `RadioGroup`/`RadioItem`, `Sub`/`SubTrigger`/`SubContent`) exactly the way `## context-menu` already reused it — same DOM-nested-not-portalled submenu ADAPT, same server-rendered `checked bool` MECHANISM, same checkbox-does-not-close/radio-does-close split, same root-not-nearest-content `hidePopover()` fix for "selecting an item inside a submenu closes the whole menu" (`contentOf(item)` here walks to `[data-gsxui-menubar-menu]`, the per-menu pairing wrapper — NOT `[data-gsxui-menubar]`, the whole bar, and NOT `item.closest(CONTENT_SELECTOR)`, which would resolve to the nearest SUB-content from inside a submenu). Hooks are `data-gsxui-menubar-*` throughout, never a prefix shared with `dropdown.js`'s own `data-gsxui-dropdown-*` or `context-menu.js`'s own `data-gsxui-contextmenu-*` — see `## dropdown`'s own MECHANISM entry (fix round 1) for the incident this namespacing avoids repeating. Mechanically verified disjoint: a Node script imports every `ui/*.js` with `on()` shimmed to record every resolved `(type, capture, selector)` triple and asserts no triple is registered by more than one file — 116 registrations across 20 behavior modules, zero collisions (a deliberately-collided run against the same script, temporarily renaming a menubar selector to a dropdown one, was confirmed to fail loudly first, ruling out a false-negative checker).
...
- ADAPT: `data-side="bottom"` is server-rendered statically, the same no-runtime-side-tracking substitution as every sibling popover's own ADAPT (`## dropdown`'s own NOTE) — `ui/navigation-menu.js` always opens below the trigger.
...
- Registry: `navigation-menu.gsx` imports `ui/icon` (`NavigationMenuTrigger`'s `ChevronDown`) — `registry.Deps("navigation-menu")` is `["icon"]`, same shape as `## dropdown`'s/`## context-menu`'s/`## menubar`'s own icon dependency. `HasJS("navigation-menu")` is `true` (`ui/navigation-menu.js`).
