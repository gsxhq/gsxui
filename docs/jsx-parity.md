...
- GAP: multi-month (`numberOfMonths`), week numbers (`showWeekNumber`), custom modifiers, locales/i18n, and the footer (`props.footer`) are not ported. For i18n specifically: `Intl`/`toLocaleString` on the client alone is not a viable substitute, even though the browser has locale APIs the server-rendered Go code does not reach for — the calendar's grid, weekday headers, and captions are rendered once by the server and then updated *in place* by the client on every navigation (the same single-authority mechanism the fixed six-row grid exists to enable), so the month/weekday names the client writes on navigation must be byte-identical to the ones the server already wrote on first paint, or a client-side `Intl` call using the browser's own locale would silently disagree with the server's hardcoded English names the moment the two differ (a non-English browser locale, or a server locale param this port doesn't have) — the fix would be a real locale parameter threaded through both `calendarMonthNames`/`d.Format(...)` in `ui/calendar.gsx` and the JS twin's own hardcoded `MONTH_NAMES`/weekday array in `ui/calendar.js`, not a client-only `Intl` call. Multi-month, week numbers, custom modifiers and the footer are plain scope cuts with no equivalent mechanism at all yet, ledgered here rather than reproduced.
- Registry: `calendar.gsx` imports `ui/button`, `ui/icon` (the nav chevrons), and `ui/native-select` (the dropdown caption's `<select>` pair) — `registry.Deps("calendar")` is `["button", "icon", "native-select"]`, pinned in `internal/registry/registry_test.go`. `HasJS("calendar")` is `true` (`ui/calendar.js`).
