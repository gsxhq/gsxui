...
- MECHANISM (checkbox vs radio close behavior): selecting a checkbox item flips it in place and does NOT close the menu (Radix's own contract); selecting a radio item DOES close the menu, same as a plain `Item`. `data-gsxui-menu-radio-group` scopes the "clear every other item" sibling walk to one radio group, so a page with more than one group doesn't cross-clear.
