...
- ADAPT (nova erases new-york-v4's own two per-component menubar deltas): new-york-v4's `MenubarCheckboxItem`/`MenubarRadioItem` carry `rounded-xs`, genuinely different from dropdown/context-menu's own `rounded-sm`; `MenubarSubTrigger` carries NO `gap-*` token at all (context-menu's own SubTrigger at least had `items-center` immediately followed by `rounded-sm`, no gap either, before nova's own harmonization — menubar's is the same shape). nova's own `style-nova.css` independently erases both: `.cn-menubar-checkbox-item`/`.cn-menubar-radio-item` land on the same `rounded-md` every item-shaped part in this whole menu family uses, and `.cn-menubar-sub-trigger` carries `gap-1.5`, same as dropdown/context-menu's own SubTrigger — the result is that `MenubarItem`/`MenubarCheckboxItem`/`MenubarRadioItem`/`MenubarSubTrigger` all end up BYTE-IDENTICAL, modulo the `data-slot`/`data-gsxui-*` prefix, to their already-shipped `DropdownMenuItem`/`CheckboxItem`/`RadioItem`/`SubTrigger` pinned classes (see `ui/menubar_test.go`'s own pinned tests). `MenubarTrigger` is the one exception that does NOT get bumped to `rounded-md` — nova's own `.cn-menubar-trigger` literally keeps `rounded-sm`, a genuine per-part nova value, not an oversight.
