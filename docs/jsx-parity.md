...
- GAP (shared-viewport mode not shipped, FIX ROUND 1 — supersedes an earlier version of this ledger entry that described a v1 architecture, reverted): shadcn's own `NavigationMenu` takes a `viewport` prop, defaulting true, which portals every `NavigationMenuContent` into one shared `NavigationMenuViewport` panel that morphs between differently-sized menus. This port ships ONLY the `viewport={false}` configuration — new-york-v4's own OTHER first-class supported mode, with its own real class strings (source map `## navigation-menu` §1's own `group-data-[viewport=false]/navigation-menu:` block), not an invented fallback. `NavigationMenuContent` is unconditionally its own fully-chromed panel (border/bg/shadow/rounded baked in, the `group-data-[viewport=false]/navigation-menu:` prefix stripped from every token since there is no second mode left to gate against — the same "drop the selector, don't ship dead CSS" call as `## dropdown`'s own `inset` ADAPT). `NavigationMenuViewport` is DROPPED entirely, not kept as a no-op — the `viewport={false}` path this port ships never renders it in upstream shadcn either (`{viewport && <NavigationMenuViewport />}`), so there was nothing left for it to do; shipping it anyway would have repeated the "hook nothing binds to" mistake `## dropdown`'s own history already caught once in this batch.
  ...
  - Two reasons this is not revisited for a v2 attempt at the shared mode within this task: (1) gsx has no portal, and reproducing Radix's Content-portals-into-Viewport model without one means moving a live popover between DOM parents at runtime (a real option — `appendChild` genuinely moves a node without losing its listeners — deliberately not attempted here, since it needs reparenting bookkeeping this shape avoids entirely). (2) The shared viewport's only real visual advantage is the morph between differently-sized panels, and the source map's own decisive finding (`## navigation-menu` §3, unchanged by this fix) already established that none of the eight nova-family stylesheets transitions width or height on the viewport — only a scale transform on open/close. The shared viewport buys essentially nothing this port doesn't already get from each `Content`'s own independent open/close scale transition.
- MECHANISM (positioning): `ui/navigation-menu.js` positions each `Content` under ITS OWN trigger's `getBoundingClientRect()` (`top-full`/`mt-1.5`-equivalent, matching new-york-v4's own `viewport={false}` anchoring, which targets the trigger's own `NavigationMenuItem` — `class="relative"` — not a shared panel location), the same hand-rolled-fixed-position stopgap every sibling popover in this codebase uses (`## dropdown`'s own NOTE) — `position:fixed` overrides the authored `top-0`/`left-0`/`top-full` tokens regardless, since a popover promoted to the top layer is not positioned relative to its DOM ancestor even when authored `absolute`.
- GAP: the six-token `data-[motion=…]`/`animate-in`/`animate-out`/`fade-in`/`fade-out` direction-aware slide on `NavigationMenuContent` (new-york-v4's own "which side did this panel enter from" animation, driven by Radix's own runtime index tracking, `derived-not-read`) is not reproduced. Per the task's own binding decision ("the popover family's discrete-transition block is reused byte-identically"), `Content` gets the identical discrete-transition block every other popover in this codebase carries (`## animations`) instead — the same six-tokens-replaced-by-one-mechanism RULING `## menubar`'s own `MenubarContent` entry already made for its own (differently-shaped) omission. Reusing that block requires `Content` to genuinely be `popover`-backed (the `open:`/`starting:open:` Tailwind variants only match `:popover-open`), which is itself the reason `Content` is independently a popover rather than a plain toggled `<div>`.
...
- ADAPT (nova, `NavigationMenuTriggerStyle`/`NavigationMenuTrigger`): `rounded-md` → `rounded-lg` (metric). `bg-background` is dropped entirely — nova's own `.cn-navigation-menu-trigger` carries no background-color utility at all, a real, not just metric, style choice per the source map's own explicit finding ("port nova's version as written… don't silently reintroduce `bg-background`"). Nova's FURTHER hover-model rewrite (`hover:bg-muted`/`focus:bg-muted`, `data-open:` shorthand replacing `data-[state=open]:`) is NOT adopted — a real interaction-model/color change, out of scope the same way `## menubar`'s own `MenubarTrigger` ADAPT already ruled for the identical situation; the standing house exception keeps `data-[state=…]:` everywhere in this codebase, and colors are untouched by a metric-tokens-only nova retarget except where a source-map finding explicitly says otherwise (as it does, narrowly, for `bg-background` alone). The chevron keeps new-york-v4's own `group-data-[state=open]:` selector, not nova's own `group-data-open:`/`group-data-popup-open:` pair — the second of which belongs to Base UI's differently-shaped primitive per the source map's own provenance note and has no Radix `data-state` equivalent to key off here.
