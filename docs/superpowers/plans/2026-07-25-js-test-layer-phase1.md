...
- **No global animation disabling.** Do not inject `* { transition: none !important }`. `dialog.js` awaits finite own Web Animations while `sonner.js` waits on `transitionend` with a 600ms fallback cap; disabling animations erases dialog's exit boundary and pushes Sonner onto its cap, hiding real timing bugs. Use Playwright's retrying assertions instead.
