...
// components: ui/toaster.gsx is the positioned region, ui/toast.gsx is the
// card, and the card is usable standalone (a server-rendered flash row).
// gsxui renders the toast DOM itself rather than delegating to the sonner
// npm package as upstream shadcn's sonner.tsx does. Upstream shadcn has
// since added a matching standalone "toast" component
// (apps/v4/registry/bases/base/ui/toast.tsx, PR #11266) using the same slot
// vocabulary rendered here. The split lets each component's slots follow the
