...
func GenerateAccessors(shape recipe.Shape) ([]byte, error) {
	...
	// The registry name is the CSS identity and may be hyphenated; the type name
	// is derived from it. See componentIdentifier.
	typeName, err := componentRecipeTypeName(shape.Component)
	if err != nil {
		return nil, err
	}
