...
component DrawerContent(direction string, children gsx.Node, attrs gsx.Attrs) {
	...
	>
		{ if direction == "" || direction == "bottom" {
			<div
				class={ "bg-muted mx-auto mt-4 hidden h-1.5 w-[100px] shrink-0 rounded-none" }
...
component DrawerHeader(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-1 p-4 md:gap-2 md:text-left flex flex-col" } { attrs... } data-gsxui-slot-drawer-header>
		{ children }
	</div>
}
...
component DrawerTitle(children gsx.Node, attrs gsx.Attrs) {
	<h2
		class={ "text-foreground text-lg font-semibold uppercase tracking-wider" }
		{ attrs... }
...
component DrawerDescription(children gsx.Node, attrs gsx.Attrs) {
	<p
		class={ "text-muted-foreground text-sm leading-relaxed mt-0.5 text-balance" }
		{ attrs... }
