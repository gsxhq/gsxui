...
component Command(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "size-full overflow-hidden", "bg-popover text-popover-foreground flex flex-col" }
		{ attrs... }
...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "p-0 [dialog&]:overflow-hidden top-1/3 translate-y-0" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"m-1 px-3 h-10 border border-transparent border-b-input bg-transparent [&>svg]:size-3.5 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2"
		}
	...
	>
		...
		<input
			...
			class={ "w-full text-sm px-2 flex" }
			{ attrs... }
...
component CommandGroup(heading string, children gsx.Node, attrs gsx.Attrs) {
	<div
		role="group"
		class={
			"text-foreground **:[[cmdk-group-heading]]:text-muted-foreground overflow-hidden p-1.5 **:[[cmdk-group-heading]]:px-3 **:[[cmdk-group-heading]]:py-2 **:[[cmdk-group-heading]]:text-xs **:[[cmdk-group-heading]]:font-semibold **:[[cmdk-group-heading]]:uppercase **:[[cmdk-group-heading]]:tracking-wider"
		}
		{ attrs... }
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-none px-3 py-2 text-sm outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-none [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0"
		}
		{ attrs... }
