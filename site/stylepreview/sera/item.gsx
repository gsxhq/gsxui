...
component Item(variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"[a]:hover:bg-muted rounded-none border text-sm flex flex-wrap",
			switch variant {
			case "outline":
				"border-border"
			case "muted":
...
component ItemTitle(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2 text-xs leading-snug font-semibold uppercase underline-offset-4 flex" }
		{ attrs... }
...
component ItemDescription(children gsx.Node, attrs gsx.Attrs) {
	<p class={ "text-muted-foreground text-left text-sm leading-relaxed" } { attrs... } data-gsxui-slot-item-description>
		{ children }
	</p>
}
