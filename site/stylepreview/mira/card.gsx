...
component CardHeader(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-1 rounded-t-lg px-(--card-spacing) grid auto-rows-min grid-rows-[auto_auto]" }
		{ attrs... }
...
component CardFooter(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "rounded-b-lg px-(--card-spacing) flex" } { attrs... } data-gsxui-slot-card-footer>{ children }</div>
}
