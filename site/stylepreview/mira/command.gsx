...
component Command(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "size-full overflow-hidden", "bg-popover text-popover-foreground rounded-xl p-1 flex flex-col" }
		{ attrs... }
...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "rounded-xl p-0 [dialog&]:overflow-hidden top-1/3 translate-y-0" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"mx-1 mt-1 pl-2 bg-input/20 dark:bg-input/30 border border-input h-8 rounded-md [&>svg]:size-3.5 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2"
		}
	...
	>
		...
		<input
			...
			class={ "w-full text-xs/relaxed flex" }
			{ attrs... }
...
component CommandEmpty(children gsx.Node, attrs gsx.Attrs) {
	<div hidden class={ "py-6 text-center text-xs/relaxed" } { attrs... } data-gsxui-slot-command-empty>{ children }</div>
}
...
component CommandSeparator(attrs gsx.Attrs) {
	<div role="separator" class={ "bg-border/50 -mx-1 my-1 h-px" } { attrs... } data-gsxui-slot-command-separator></div>
}
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex min-h-7 cursor-default items-center gap-2 rounded-md px-2.5 py-1.5 text-xs/relaxed outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-md [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0"
		}
		{ attrs... }
