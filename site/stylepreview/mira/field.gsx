...
component FieldLegend(variant string, children gsx.Node, attrs gsx.Attrs) {
	<legend
		...
		class={ "mb-2 font-medium", switch variant { case "label": "text-xs/relaxed" default: "text-sm" } }
		{ attrs... }
...
component FieldGroup(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"group/field-group", "gap-4 [&>[data-gsxui-slot-field-group]]:gap-4 flex flex-col w-full @container/field-group"
		}
		{ attrs... }
...
component FieldLabel(children gsx.Node, attrs gsx.Attrs) {
	<Label
		class={
			"group/field-label",
			"has-[input:checked]:bg-primary/5 dark:has-[input:checked]:bg-primary/10 gap-2 leading-snug has-[>[data-gsxui-slot-field]]:rounded-md has-[>[data-gsxui-slot-field]]:border has-[>[data-gsxui-slot-field]]:not-has-[:disabled,[data-disabled]]:hover:bg-input/40 has-[>[data-gsxui-slot-field]]:has-[:focus-visible]:border-ring has-[>[data-gsxui-slot-field]]:has-[:focus-visible]:ring-ring/30 has-[>[data-gsxui-slot-field]]:has-[:focus-visible]:ring-2 flex w-fit has-[>[data-gsxui-slot-field]]:w-full has-[>[data-gsxui-slot-field]]:flex-col [&>[data-gsxui-slot-field]]:p-2"
		}
		{ attrs... }
...
component FieldTitle(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2 text-xs/relaxed font-medium flex w-fit items-center" }
		{ attrs... }
...
component FieldSeparator(children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={ "-my-2 h-5 text-xs/relaxed group-data-[variant=outline]/field-group:-mb-2 relative" }
		{ attrs... }
...
component FieldError(children gsx.Node, attrs gsx.Attrs) {
	{ if children != nil {
		<div role="alert" class={ "text-destructive text-xs/relaxed" } { attrs... } data-gsxui-slot-field-error>
			{ children }
		</div>
	} }
}
