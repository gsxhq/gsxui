...
component AccordionTrigger(children gsx.Node, attrs gsx.Attrs) {
	<summary
		class={ "gap-6 p-2 text-left text-xs/relaxed font-medium hover:underline flex" }
		{ attrs... }
...
component AccordionContent(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up px-2 text-xs/relaxed" }
		{ attrs.Without("class")... }
