...
component EmptyHeader(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-1 flex flex-col max-w-sm items-center text-center" } { attrs... } data-gsxui-slot-empty-header>
		{ children }
	</div>
}
...
component EmptyContent(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2 text-xs/relaxed flex flex-col w-full max-w-sm min-w-0 items-center text-balance" }
		{ attrs... }
