...
//line item.gsx:51:2
	...
	_gsxv0 := "group/item"
	_gsxv1 := "[a]:hover:bg-muted rounded-md border text-xs/relaxed flex flex-wrap"
	var _gsxv2 string
	switch variant {
	case "outline":
...
//line item.gsx:85:2
	...
	case "image":
		_gsxv4 = "size-8 overflow-hidden rounded-sm group-data-[size=sm]/item:size-8 group-data-[size=xs]/item:size-6 [&_img]:size-full [&_img]:object-cover"
	default:
...
//line item.gsx:118:2
	_gsxgw.S("<div class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("gap-2 text-xs/relaxed leading-snug font-medium underline-offset-4 flex"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
...
//line item.gsx:131:2
	_gsxgw.S("<p class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("text-muted-foreground text-left text-xs/relaxed"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
