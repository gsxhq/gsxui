...
component SelectTrigger(size string, children gsx.Node, attrs gsx.Attrs) {
	<button
		...
		class={
			...
			switch size { case "sm": "h-6" default: "h-7" }
		}
		{ attrs... }
...
component SelectLabel(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "text-muted-foreground px-2 py-1.5 text-xs" } { attrs... } data-gsxui-slot-select-label>
		{ children }
	</div>
}
...
component SelectItem(value string, selected bool, disabled bool, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground min-h-7 gap-2 rounded-md px-2 py-1 text-xs/relaxed [&_svg:not([class*='size-'])]:size-3.5 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 flex data-[state=checked]:[&>[data-gsxui-slot-select-item-indicator]]:flex"
		}
		{ attrs... }
		data-gsxui-slot-select-item
	>
		<span
			class={ "pointer-events-none absolute right-2 hidden items-center justify-center" }
...
component SelectSeparator(attrs gsx.Attrs) {
	<div aria-hidden="true" class={ "bg-border/50 -mx-1 my-1 h-px" } { attrs... } data-gsxui-slot-select-separator></div>
}
