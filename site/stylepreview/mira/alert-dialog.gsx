...
component AlertDialogContent(children gsx.Node, attrs gsx.Attrs) {
	<DialogContent
		class={
			...
			"transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 bg-popover text-popover-foreground ring-foreground/10 gap-3 rounded-xl p-4 ring-1 duration-100 max-w-xs sm:max-w-sm"
		}
...
component AlertDialogHeader(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"grid grid-rows-[auto_1fr] place-items-center gap-1 text-center sm:group-data-[size=default]/alert-dialog-content:place-items-start sm:group-data-[size=default]/alert-dialog-content:text-left"
		}
		{ attrs... }
...
component AlertDialogTitle(children gsx.Node, attrs gsx.Attrs) {
	<h2 class={ "text-sm font-medium" } { attrs... } data-gsxui-slot-alert-dialog-title data-gsxui-slot-dialog-title>
		{ children }
	</h2>
}
