...
component Item(variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"[a]:hover:bg-muted rounded-none border text-xs flex flex-wrap",
			switch variant {
			case "outline":
				"border-border"
			case "muted":
...
component ItemTitle(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-2 text-xs font-medium underline-offset-4 flex" } { attrs... } data-gsxui-slot-item-title>
		{ children }
	</div>
}
...
component ItemDescription(children gsx.Node, attrs gsx.Attrs) {
	<p
		class={ "text-muted-foreground text-left text-xs/relaxed group-data-[size=xs]/item:text-xs/relaxed" }
		{ attrs... }
