...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"border-b pb-0 bg-input/30 border-input/30 h-8 border-none shadow-none [&>[data-gsxui-slot-input-group-addon]]:pl-2 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex"
		}
...
component CommandEmpty(children gsx.Node, attrs gsx.Attrs) {
	<div hidden class={ "py-6 text-center text-xs" } { attrs... } data-gsxui-slot-command-empty>{ children }</div>
}
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		aria-selected="false"
		class={
			"data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-none px-2 py-2 text-xs outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-none [&_svg:not([class*='size-'])]:size-4"
		}
		{ attrs... }
