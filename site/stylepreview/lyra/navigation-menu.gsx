...
component NavigationMenuTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			...
			"hover:bg-muted focus:bg-muted data-[state=open]:hover:bg-muted data-[state=open]:focus:bg-muted data-[state=open]:bg-muted/50 focus-visible:ring-ring/50 rounded-none px-2.5 py-1.5 text-xs font-medium transition-all focus-visible:ring-1 focus-visible:outline-1 disabled:opacity-50 inline-flex"
		}
		type="button"
		aria-expanded="false"
		data-state="closed"
...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"data-active:focus:bg-muted data-active:hover:bg-muted data-active:bg-muted/50 focus-visible:ring-ring/50 hover:bg-muted focus:bg-muted flex items-center gap-2 rounded-none p-2 text-xs transition-all outline-none focus-visible:ring-1 focus-visible:outline-1 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-none [&_svg:not([class*='size-'])]:size-4",
			switch linkVariant {
			case "trigger":
...
component NavigationMenuIndicator(attrs gsx.Attrs) {
	...
	>
		<div class={ "bg-border rounded-none shadow-md" } data-gsxui-slot-navigation-menu-indicator-arrow></div>
	</div>
}
