// Code generated by gsx; DO NOT EDIT.

package lyra

import (
	_gsxctx "context"
	"github.com/gsxhq/gsx"
	_gsxrt "github.com/gsxhq/gsx"
	_gsxstd "github.com/gsxhq/gsx/std"
	_gsxcm "github.com/gsxhq/gsxui/merge"
	_gsxio "io"
)

//line drawer.gsx:5:1
// Drawer composes Dialog's root and behavior while exposing a directional
// side-anchored content role. Drag-to-dismiss remains outside this component.
//
// direction is physical, matching shadcn: direction="left"/"right" always
// anchors to that physical viewport edge (position, border, corner radius
// and slide direction), in both dir="ltr" and dir="rtl" documents.
// direction="top"/"bottom" is unaffected by text direction. Interior
// presentation — header/footer padding and title/description text
// alignment — is logical and follows dir normally.
//
// DrawerContent deliberately does NOT reuse Dialog's content presentation —
// it must not inherit the plain-modal box — so it carries the shared
// <dialog> chrome as its own recipe utilities instead. Every class attribute
// below is resolved to concrete utilities at generation time.

//line drawer.gsx:19:1
func Drawer(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawer(ctx, _gsxgw, children, attrs)
	})
}

func _gsxrenderDrawer(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:20:2
	_gsxgw.NodeResult(_gsxrenderDialog(ctx, _gsxgw, _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
//line drawer.gsx:20:46
		_gsxgw.Node(ctx, children)
		return _gsxgw.Err()
	}), _gsxrt.ConcatAttrs(attrs, _gsxrt.Attrs{{Key: "data-gsxui-slot-drawer", Value: _gsxrt.Toggle(true)}})))
	return _gsxgw.Err()
}

//line drawer.gsx:23:1
func DrawerTrigger(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
//line drawer.gsx:24:2
		_gsxgw.S("<button")
		if !attrs.Has("type") {
			_gsxgw.S(" type=\"button\"")
		}
		if !attrs.Has("aria-haspopup") {
			_gsxgw.S(" aria-haspopup=\"dialog\"")
		}
		if !attrs.Has("aria-expanded") {
			_gsxgw.S(" aria-expanded=\"false\"")
		}
		_gsxgw.ClassMerged(_gsxcm.Merge, attrs.Class())
		_gsxgw.StyleMerged("", attrs.Style())
		_gsxgw.Spread(ctx, "button", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-trigger"})
		_gsxgw.BoolAttr("data-gsxui-slot-drawer-trigger", true)
		_gsxgw.S(">")
//line drawer.gsx:31:3
		_gsxgw.Node(ctx, children)
		_gsxgw.S("</button>")
		return _gsxgw.Err()
	})
}

//line drawer.gsx:35:1
func DrawerContent(direction string, children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawerContent(ctx, _gsxgw, direction, children, attrs)
	})
}

func _gsxrenderDrawerContent(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, direction string, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:36:2
	_gsxgw.S("<dialog")
	_gsxv0 := "backdrop:bg-black/10 supports-backdrop-filter:backdrop:backdrop-blur-xs bg-popover text-popover-foreground flex h-auto flex-col text-xs/relaxed data-[vaul-drawer-direction=bottom]:rounded-none data-[vaul-drawer-direction=bottom]:border-t data-[vaul-drawer-direction=left]:rounded-none data-[vaul-drawer-direction=left]:border-r data-[vaul-drawer-direction=right]:rounded-none data-[vaul-drawer-direction=right]:border-l data-[vaul-drawer-direction=top]:rounded-none data-[vaul-drawer-direction=top]:border-b m-0 gap-4 shadow-lg transition ease-in-out backdrop:transition-none fixed z-50 duration-200 outline-none data-[state=closed]:animate-out data-[state=open]:animate-in backdrop:backdrop-blur-xs backdrop:duration-200 data-[state=open]:backdrop:animate-in data-[state=open]:backdrop:fade-in-0 data-[state=closed]:backdrop:animate-out data-[state=closed]:backdrop:fade-out-0 open:flex"
	var _gsxv1 string
	switch direction {
	case "left":
		_gsxv1 = "inset-y-0 left-0 right-auto h-full max-h-none w-3/4 rounded-r-xl border-r sm:max-w-sm data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left md:[&_[data-gsxui-slot-drawer-header]]:text-start"
	case "right":
		_gsxv1 = "inset-y-0 right-0 left-auto h-full max-h-none w-3/4 rounded-l-xl border-l sm:max-w-sm data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right md:[&_[data-gsxui-slot-drawer-header]]:text-start"
	case "top":
		_gsxv1 = "inset-x-0 top-0 bottom-auto mb-24 h-auto max-h-[80vh] w-full max-w-none rounded-b-xl border-b data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top [&_[data-gsxui-slot-drawer-header]]:text-center"
	default:
		_gsxv1 = "inset-x-0 bottom-0 top-auto mt-24 h-auto max-h-[80vh] w-full max-w-none rounded-t-xl border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom [&_[data-gsxui-slot-drawer-header]]:text-center"
	}
	_gsxgw.S(" class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class(_gsxv0), _gsxrt.Class(_gsxv1), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	if !attrs.Has("data-state") {
		_gsxgw.S(" data-state=\"closed\"")
	}
	if !attrs.Has("data-side") {
		_gsxgw.S(" data-side=\"")
		_gsxgw.AttrValue(string(_gsxstd.Default((direction), "bottom")))
		_gsxgw.S("\"")
	}
	_gsxgw.StyleMerged("", attrs.Style())
	_gsxgw.Spread(ctx, "dialog", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-content", "data-gsxui-slot-dialog-content"})
	_gsxgw.BoolAttr("data-gsxui-slot-drawer-content", true)
	_gsxgw.BoolAttr("data-gsxui-slot-dialog-content", true)
	_gsxgw.S(">")
//line drawer.gsx:56:3
	if direction == "" || direction == "bottom" {
//line drawer.gsx:57:4
		_gsxgw.S("<div class=\"")
		_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("bg-muted mx-auto mt-4 hidden h-1 w-[100px] shrink-0 rounded-none"))
		_gsxgw.S("\"")
		_gsxgw.BoolAttr("data-gsxui-slot-drawer-handle", true)
		_gsxgw.S("></div>")
	}
//line drawer.gsx:62:3
	_gsxgw.Node(ctx, children)
	_gsxgw.S("</dialog>")
	return _gsxgw.Err()
}

//line drawer.gsx:66:1
func DrawerHeader(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawerHeader(ctx, _gsxgw, children, attrs)
	})
}

func _gsxrenderDrawerHeader(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:67:2
	_gsxgw.S("<div class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("gap-0.5 p-4 md:gap-0.5 md:text-left flex flex-col"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
	_gsxgw.Spread(ctx, "div", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-header"})
	_gsxgw.BoolAttr("data-gsxui-slot-drawer-header", true)
	_gsxgw.S(">")
//line drawer.gsx:68:3
	_gsxgw.Node(ctx, children)
	_gsxgw.S("</div>")
	return _gsxgw.Err()
}

//line drawer.gsx:72:1
func DrawerFooter(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawerFooter(ctx, _gsxgw, children, attrs)
	})
}

func _gsxrenderDrawerFooter(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:73:2
	_gsxgw.S("<div class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("gap-2 p-4 flex flex-col"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
	_gsxgw.Spread(ctx, "div", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-footer"})
	_gsxgw.BoolAttr("data-gsxui-slot-drawer-footer", true)
	_gsxgw.S(">")
//line drawer.gsx:73:86
	_gsxgw.Node(ctx, children)
	_gsxgw.S("</div>")
	return _gsxgw.Err()
}

//line drawer.gsx:76:1
func DrawerTitle(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawerTitle(ctx, _gsxgw, children, attrs)
	})
}

func _gsxrenderDrawerTitle(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:77:2
	_gsxgw.S("<h2 class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("text-foreground text-sm font-medium"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
	_gsxgw.Spread(ctx, "h2", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-title", "data-gsxui-slot-dialog-title"})
	_gsxgw.BoolAttr("data-gsxui-slot-drawer-title", true)
	_gsxgw.BoolAttr("data-gsxui-slot-dialog-title", true)
	_gsxgw.S(">")
//line drawer.gsx:83:3
	_gsxgw.Node(ctx, children)
	_gsxgw.S("</h2>")
	return _gsxgw.Err()
}

//line drawer.gsx:87:1
func DrawerDescription(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
		return _gsxrenderDrawerDescription(ctx, _gsxgw, children, attrs)
	})
}

func _gsxrenderDrawerDescription(ctx _gsxctx.Context, _gsxgw *_gsxrt.Writer, children gsx.Node, attrs gsx.Attrs) error {
	if _gsxerr := _gsxgw.Err(); _gsxerr != nil {
		return _gsxerr
	}
//line drawer.gsx:88:2
	_gsxgw.S("<p class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("text-muted-foreground text-xs/relaxed"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
	_gsxgw.Spread(ctx, "p", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-description", "data-gsxui-slot-dialog-description"})
	_gsxgw.BoolAttr("data-gsxui-slot-drawer-description", true)
	_gsxgw.BoolAttr("data-gsxui-slot-dialog-description", true)
	_gsxgw.S(">")
//line drawer.gsx:94:3
	_gsxgw.Node(ctx, children)
	_gsxgw.S("</p>")
	return _gsxgw.Err()
}

//line drawer.gsx:98:1
func DrawerClose(children gsx.Node, attrs gsx.Attrs) _gsxrt.Node {
	return _gsxrt.Func(func(ctx _gsxctx.Context, _gsxw _gsxio.Writer) error {
		_gsxgw := _gsxrt.W(_gsxw)
//line drawer.gsx:99:2
		_gsxgw.S("<button")
		if !attrs.Has("data-gsxui-dialog-close") {
			_gsxgw.BoolAttr("data-gsxui-dialog-close", true)
		}
		if !attrs.Has("type") {
			_gsxgw.S(" type=\"button\"")
		}
		_gsxgw.ClassMerged(_gsxcm.Merge, attrs.Class())
		_gsxgw.StyleMerged("", attrs.Style())
		_gsxgw.Spread(ctx, "button", attrs, _gsxrt.AttrSinks{}, []string{"class", "style", "data-gsxui-slot-drawer-close"})
		_gsxgw.BoolAttr("data-gsxui-slot-drawer-close", true)
		_gsxgw.S(">")
//line drawer.gsx:99:90
		_gsxgw.Node(ctx, children)
		_gsxgw.S("</button>")
		return _gsxgw.Err()
	})
}
