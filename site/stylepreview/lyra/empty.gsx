...
component Empty(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"gap-4 rounded-none border-dashed p-6 flex flex-col w-full min-w-0 flex-1 items-center justify-center text-center text-balance"
		}
		{ attrs... }
...
component EmptyMedia(variant string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			...
			switch variant {
			case "icon":
				"bg-muted text-foreground flex size-8 shrink-0 items-center justify-center rounded-none [&_svg:not([class*='size-'])]:size-4"
			default:
				"bg-transparent"
			}
		}
...
component EmptyTitle(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "text-sm font-medium" } { attrs... } data-gsxui-slot-empty-title>
		{ children }
	</div>
}
...
component EmptyDescription(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "text-xs/relaxed text-muted-foreground [&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-primary" }
		{ attrs... }
...
component EmptyContent(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2.5 text-xs flex flex-col w-full max-w-sm min-w-0 items-center text-balance" }
		{ attrs... }
