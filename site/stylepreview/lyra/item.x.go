...
//line item.gsx:51:2
	...
	_gsxv0 := "[a]:hover:bg-muted rounded-none border text-xs flex flex-wrap"
	var _gsxv1 string
	switch variant {
	case "outline":
		_gsxv1 = "border-border"
	case "muted":
		_gsxv1 = "bg-muted/50 border-transparent"
	default:
		_gsxv1 = "border-transparent"
	}
	_gsxgw.S(" class=\"")
...
//line item.gsx:84:2
	...
	switch variant {
	case "icon":
		_gsxv3 = "[&_svg:not([class*='size-'])]:size-4"
	case "image":
		_gsxv3 = "size-10 overflow-hidden rounded-none group-data-[size=sm]/item:size-8 group-data-[size=xs]/item:size-6 [&_img]:size-full [&_img]:object-cover"
	default:
...
//line item.gsx:105:2
	_gsxgw.S("<div class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("gap-1 group-data-[size=xs]/item:gap-0 flex flex-col"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
