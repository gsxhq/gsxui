...
component Menubar(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "h-8 gap-0.5 rounded-none border p-1 flex" }
		role="menubar"
		{ attrs... }
...
component MenubarItem(variant string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"group/menubar-item",
			"focus:bg-accent focus:text-accent-foreground dark:data-[variant=destructive]:focus:bg-destructive/20 not-data-[variant=destructive]:focus:**:text-accent-foreground gap-2 rounded-none px-2 py-2 text-xs [&_svg:not([class*='size-'])]:size-4 flex",
			switch variant {
			case "destructive":
