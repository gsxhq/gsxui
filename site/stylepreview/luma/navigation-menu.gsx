...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"focus-visible:ring-ring/30 hover:bg-muted focus:bg-muted flex items-center gap-1.5 rounded-3xl p-3 text-sm transition-all outline-none focus-visible:ring-3 focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-2xl",
			switch linkVariant {
			case "trigger":
