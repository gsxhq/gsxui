...
component Command(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "size-full overflow-hidden", "bg-popover text-popover-foreground rounded-4xl p-1 flex flex-col" }
		{ attrs... }
...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "rounded-4xl p-0 [dialog&]:overflow-hidden top-1/3 translate-y-0" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"mx-1 mt-1 pl-2 bg-input/50 border border-transparent h-9 rounded-4xl [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2"
		}
...
component CommandSeparator(attrs gsx.Attrs) {
	<div role="separator" class={ "bg-border/50 my-1.5 h-px" } { attrs... } data-gsxui-slot-command-separator></div>
}
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-2xl px-3 py-2 text-sm font-medium outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-3xl [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
		}
		{ attrs... }
