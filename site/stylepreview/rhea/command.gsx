...
component Command(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "bg-popover text-popover-foreground rounded-3xl p-1 flex flex-col" }
		{ attrs... }
...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "rounded-3xl p-0 [dialog&]:overflow-hidden" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={ "p-1 bg-input/50 h-8 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2" }
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 min-h-7 rounded-xl px-2 py-1.5 text-sm outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-2xl [&_svg:not([class*='size-'])]:size-4"
		}
		{ attrs... }
