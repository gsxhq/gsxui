...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"mx-1 mt-1 pl-2 bg-input/30 border border-input h-9 rounded-4xl [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2"
		}
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-lg px-3 py-2 text-sm outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-2xl [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
		}
		{ attrs... }
