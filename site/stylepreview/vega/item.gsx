...
component Item(variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/item",
			"[a]:hover:bg-muted rounded-md border text-sm flex flex-wrap w-full items-center transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors",
			switch variant {
			case "outline":
				"border-border"
			case "muted":
			...
			switch size { case "sm": "gap-2.5 px-3 py-2.5" default: "gap-3.5 px-4 py-3.5" }
		}
		{ attrs... }
