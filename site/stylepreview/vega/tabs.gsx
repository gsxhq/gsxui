...
component TabsList(children gsx.Node, attrs gsx.Attrs) {
	<div
		role="tablist"
		class={ "rounded-lg p-[3px] h-9 inline-flex w-fit items-center justify-center bg-muted text-muted-foreground" }
		{ attrs... }
...
component TabsTrigger(value string, selected bool, children gsx.Node, attrs gsx.Attrs) {
	...
	<button
		...
		tabindex={tabindex}
		class={
			"gap-1.5 rounded-md border border-transparent px-2 py-1 text-sm font-medium data-[state=active]:shadow-sm [&_svg:not([class*='size-'])]:size-4 has-[>svg]:px-1.5 inline-flex relative h-[calc(100%-1px)] flex-1 items-center justify-center whitespace-nowrap text-foreground/60 transition-all hover:text-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-1 focus-visible:outline-ring disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 dark:text-muted-foreground dark:hover:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 data-[state=active]:bg-background data-[state=active]:text-foreground dark:data-[state=active]:border-input dark:data-[state=active]:bg-input/30"
		}
		{ attrs... }
