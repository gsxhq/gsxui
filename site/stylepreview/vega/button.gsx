	...
	{ if href != "" && !disabled {
		<a
			...
			class={
				...
				switch size {
				case "xs":
					"h-6 gap-1 rounded-[min(var(--radius-md),8px)] px-2 text-xs has-[>svg]:px-1.5 [&_svg:not([class*='size-'])]:size-3"
				case "sm":
					"h-8 gap-1 rounded-[min(var(--radius-md),10px)] px-2.5 has-[>svg]:px-1.5"
				case "lg":
					"h-10 gap-1.5 px-2.5 has-[>svg]:px-2"
				case "icon":
					"size-9"
				case "icon-xs":
