...
//line item.gsx:51:2
	...
	_gsxv0 := "group/item"
	_gsxv1 := "[a]:hover:bg-muted rounded-md border text-sm flex flex-wrap"
	var _gsxv2 string
	switch variant {
	case "outline":
...
//line item.gsx:85:2
	...
	case "image":
		_gsxv4 = "size-10 overflow-hidden rounded-sm group-data-[size=sm]/item:size-8 group-data-[size=xs]/item:size-6 [&_img]:size-full [&_img]:object-cover"
	default:
...
//line item.gsx:129:2
	_gsxgw.S("<p class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("text-muted-foreground text-left text-sm leading-normal group-data-[size=xs]/item:text-xs"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
