...
//line empty.gsx:32:2
	...
	switch variant {
	case "icon":
		_gsxv1 = "bg-muted text-foreground flex size-8 shrink-0 items-center justify-center rounded-lg [&_svg:not([class*='size-'])]:size-4"
	default:
...
//line empty.gsx:72:2
	_gsxgw.S("<div class=\"")
	_gsxgw.Class(_gsxcm.Merge, _gsxrt.Class("gap-2.5 text-sm flex flex-col w-full max-w-sm min-w-0 items-center text-balance"), _gsxrt.Class(attrs.Class()))
	_gsxgw.S("\"")
	_gsxgw.StyleMerged("", attrs.Style())
