...
component NativeSelect(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"group/native-select",
			"relative w-fit [&>svg]:pointer-events-none [&>svg]:absolute [&>svg]:opacity-50 [&>svg]:text-muted-foreground [&>svg]:top-1/2 [&>svg]:end-2.5 [&>svg]:size-4 [&>svg]:-translate-y-1/2 has-[select:disabled]:opacity-50",
			attrs.Class()
		}
