...
component NavigationMenuTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			...
			"hover:bg-muted focus:bg-muted data-[state=open]:hover:bg-muted data-[state=open]:focus:bg-muted data-[state=open]:bg-muted/50 focus-visible:ring-ring/50 rounded-lg px-2.5 py-1.5 text-sm font-medium transition-all focus-visible:ring-3 focus-visible:outline-1 disabled:opacity-50 inline-flex h-9 w-max items-center justify-center outline-none disabled:pointer-events-none"
		}
		type="button"
		aria-expanded="false"
		data-state="closed"
	...
	>
		...
		<icon.ChevronDown
			class={
				"relative top-px ms-1 size-3 transition duration-300 group-data-[state=open]/navigation-menu-trigger:rotate-180"
			}
...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"data-[active=true]:focus:bg-accent data-[active=true]:hover:bg-accent data-[active=true]:bg-accent/50 data-[active=true]:text-accent-foreground focus-visible:ring-ring/50 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex flex-col gap-1 rounded-lg p-2 text-sm transition-all outline-none focus-visible:ring-3 focus-visible:outline-1 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-md [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
			switch linkVariant {
			case "trigger":
			...
			switch linkActive {
			case "true":
				"bg-accent/50 text-accent-foreground hover:bg-accent hover:text-accent-foreground focus:bg-accent"
			default:
				"outline-none"
			}
		}
...
component NavigationMenuIndicator(attrs gsx.Attrs) {
	...
	>
		<div
			class={ "bg-border rounded-ss-sm shadow-md relative top-[60%] h-2 w-2 rotate-45" }
