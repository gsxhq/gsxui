...
//line alert.gsx:9:2
		...
		_gsxv0 := "group/alert"
		_gsxv1 := "grid gap-0.5 rounded-lg border px-2.5 py-2 text-start text-sm has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4 w-full"
		var _gsxv2 string
		switch variant {
		case "destructive":
