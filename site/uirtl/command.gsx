...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "rounded-xl [dialog&]:overflow-hidden p-0 top-1/3 translate-y-0" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={
			"mx-1 mt-1 ps-2 bg-input/30 border-input/30 border h-8 rounded-lg shadow-none [&>[data-gsxui-slot-input-group-addon]]:ps-2 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2"
		}
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-lg [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
		}
		{ attrs... }
...
component CommandShortcut(children gsx.Node, attrs gsx.Attrs) {
	<span
		class={ "text-muted-foreground group-data-selected/command-item:text-foreground ms-auto text-xs tracking-widest" }
		{ attrs... }
