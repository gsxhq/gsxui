...
- Name: jstest/specs/foundation-contract.spec.ts >> foundation mode excludes the default style and preserves overlay lifecycle
- Location: jstest/specs/foundation-contract.spec.ts:6:1
...
Call log:
  - navigating to "/x/dialog/basic?css=foundation", waiting until "load"
...
```ts
  ...
  2   | 
  3   | const foundation = (path: string) =>
  4   |   `${path}${path.includes("?") ? "&" : "?"}css=foundation`;
  5   | 
  6   | test("foundation mode excludes the default style and preserves overlay lifecycle", async ({
  7   |   page,
  8   | }) => {
> 9   |   await page.goto(foundation("/x/dialog/basic"));
      |              ^ Error: page.goto: Protocol error (Page.navigate): Cannot navigate to invalid URL
  10  |   expect(
  11  |     await page.evaluate(() =>
  12  |       getComputedStyle(document.documentElement)
  13  |         .getPropertyValue("--gsxui-default-style")
  14  |         .trim(),
  15  |     ),
  16  |   ).toBe("");
  17  | 
  18  |   const dialog = page.locator("[data-gsxui-dialog-content]");
  19  |   await expect(dialog).not.toHaveAttribute("open");
  20  |   await expect.poll(() => dialog.evaluate((el) => getComputedStyle(el).display)).toBe("none");
  21  | 
  22  |   await page.getByRole("button", { name: "Delete account" }).click();
  23  |   await expect(dialog).toHaveAttribute("open");
  24  |   await expect(dialog).toHaveAttribute("data-state", "open");
  25  |   expect(
  26  |     await dialog.evaluate((el) => {
  27  |       const rect = el.getBoundingClientRect();
  28  |       return rect.width > 0 && rect.height > 0;
  29  |     }),
  30  |   ).toBe(true);
  31  | 
  32  |   await page.locator("[data-gsxui-dialog-close]").first().click();
  33  |   await expect(dialog).not.toHaveAttribute("open");
  34  |   await expect(dialog).toHaveAttribute("data-state", "closed");
  35  | 
  36  |   await page.goto(foundation("/x/popover/basic"));
  37  |   const popover = page.locator("[data-gsxui-popover-content]");
