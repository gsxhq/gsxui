...
// data-gsxui-slot-chart-legend element server-side (see ChartLegendContent
...
// hidden server-authored chrome (a <template> ui/chart.render.js reads its
// class attributes off, on first hover) whenever a ChartTooltip is
// registered, so "tooltip" lands as a real slot now: its class={
// chart.Tooltip() } accessor call sits in that template's real markup,
// satisfying stylegen's checkShapeCoverage ("shape declares slot %q but
// the component never renders it" — internal/stylegen/generate.go — no
// longer applies once something renders it).
