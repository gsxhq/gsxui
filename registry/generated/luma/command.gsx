...
component CommandDialog(title string, description string, trigger gsx.Node, children gsx.Node, attrs gsx.Attrs) {
	<Dialog data-gsxui-slot-command-dialog>
		...
		<DialogContent
			...
			class={ "rounded-4xl p-0 [dialog&]:overflow-hidden" }
			{ attrs... }
...
component CommandInput(placeholder string, attrs gsx.Attrs) {
	<div
		class={ "p-1 pb-0 bg-input/50 h-9 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:opacity-50 flex items-center gap-2" }
...
component CommandItem(value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/command-item",
			"data-disabled:pointer-events-none data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground relative flex cursor-default items-center gap-2 rounded-2xl px-3 py-2 text-sm font-medium outline-hidden select-none [[data-gsxui-slot-dialog-content]_&]:rounded-3xl [&_svg:not([class*='size-'])]:size-4"
		}
		{ attrs... }
