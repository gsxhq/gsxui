...
component Menubar(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "h-9 rounded-3xl border p-1 flex" }
		role="menubar"
		{ attrs... }
...
component MenubarTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={ "hover:bg-muted aria-expanded:bg-muted rounded-2xl px-2 py-0.75 text-sm font-medium flex" }
		type="button"
		aria-haspopup="menu"
		aria-expanded="false"
		data-state="closed"
...
component MenubarItem(variant string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground dark:data-[variant=destructive]:focus:bg-destructive/20 not-data-[variant=destructive]:focus:**:text-accent-foreground gap-2.5 rounded-2xl px-3 py-2 text-sm font-medium data-disabled:opacity-50 data-inset:pl-9.5 [&_svg:not([class*='size-'])]:size-4 flex",
			switch variant {
			case "destructive":
...
component MenubarCheckboxItem(checked bool, value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground gap-2.5 rounded-2xl py-2 pr-3 pl-9.5 text-sm font-medium data-inset:pl-9.5 flex"
		}
...
component MenubarRadioItem(checked bool, value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground gap-2.5 rounded-2xl py-2 pr-3 pl-9.5 text-sm font-medium data-disabled:opacity-50 data-inset:pl-9.5 [&_svg:not([class*='size-'])]:size-4 flex"
		}
		role="menuitemradio"
		data-value={value}
		{ if checked {
