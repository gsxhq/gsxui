...
component ToggleGroup(groupType string, variant string, size string, spacing string, children gsx.Node, attrs gsx.Attrs) {
	...
	<div
		...
		class={
			"group/toggle-group",
			"rounded-md flex w-fit items-center gap-[--spacing(var(--gap))]",
			switch size { case "sm": "rounded-[min(var(--radius-md),8px)]" case "lg": "rounded-lg" default: "rounded-lg" }
		}
		{ attrs... }
...
component ToggleGroupItem(groupType string, variant string, size string, spacing string, pressed bool, value string, children gsx.Node, attrs gsx.Attrs) {
	...
	<button
		...
		class={
			"hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive disabled:pointer-events-none disabled:opacity-50 data-[state=on]:bg-muted data-[variant=outline]:border-input data-[variant=outline]:hover:bg-muted data-[variant=outline]:border gap-1 rounded-md text-xs font-medium transition-all [&_svg:not([class*='size-'])]:size-4 group-data-[spacing=0]/toggle-group:rounded-none group-data-[spacing=0]/toggle-group:px-2 group-data-horizontal/toggle-group:data-[spacing=0]:first:rounded-l-md group-data-vertical/toggle-group:data-[spacing=0]:first:rounded-t-md group-data-horizontal/toggle-group:data-[spacing=0]:last:rounded-r-md group-data-vertical/toggle-group:data-[spacing=0]:last:rounded-b-md shrink-0 focus:z-10 focus-visible:z-10 items-center justify-center whitespace-nowrap outline-none focus-visible:ring-[3px] [&_svg]:pointer-events-none [&_svg]:shrink-0",
			switch size {
			case "sm":
				"h-6 min-w-6 rounded-[min(var(--radius-md),8px)] px-2 text-[0.625rem] has-[>svg]:px-1.5 [&_svg:not([class*='size-'])]:size-3 group-data-[spacing=0]/toggle-group:has-[>svg]:px-1.5"
			case "lg":
				"h-8 min-w-8 px-2.5 has-[>svg]:px-2 group-data-[spacing=0]/toggle-group:has-[>svg]:px-2"
			default:
				"h-7 min-w-7 px-2 has-[>svg]:px-1.5 group-data-[spacing=0]/toggle-group:has-[>svg]:px-1.5"
			},
			switch sp {
			case "2":
				"isolate"
