...
component ComboboxItem(value string, selected bool, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm [&_svg:not([class*='size-'])]:size-4 flex data-[state=checked]:[&>[data-gsxui-slot-combobox-item-indicator]]:flex"
		}
		{ attrs... }
