...
component Menubar(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "h-9 gap-1 rounded-md border p-1 shadow-xs flex" }
		role="menubar"
		{ attrs... }
...
component MenubarTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={ "hover:bg-muted aria-expanded:bg-muted rounded-sm px-2 py-1 text-sm font-medium flex" }
		type="button"
		aria-haspopup="menu"
		aria-expanded="false"
		data-state="closed"
...
component MenubarItem(variant string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground dark:data-[variant=destructive]:focus:bg-destructive/20 not-data-[variant=destructive]:focus:**:text-accent-foreground gap-2 rounded-sm px-2 py-1.5 text-sm data-disabled:opacity-50 data-inset:pl-8 [&_svg:not([class*='size-'])]:size-4 flex",
			switch variant {
			case "destructive":
...
component MenubarCheckboxItem(checked bool, value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground gap-2 rounded-md py-1.5 pr-2 pl-8 text-sm data-inset:pl-8 flex"
		}
	...
	>
		<span
			class={ "left-2 size-4 [&_svg:not([class*='size-'])]:size-4" }
...
component MenubarRadioItem(checked bool, value string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground gap-2 rounded-md py-1.5 pr-2 pl-8 text-sm data-disabled:opacity-50 data-inset:pl-8 [&_svg:not([class*='size-'])]:size-4 flex"
		}
		role="menuitemradio"
		data-value={value}
		{ if checked {
	...
	>
		<span class={ "left-2 size-4 [&_svg:not([class*='size-'])]:size-4" } data-gsxui-slot-menubar-radio-item-indicator>
			<icon.Circle/>
		</span>
		{ children }
	</div>
