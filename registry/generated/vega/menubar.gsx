...
component MenubarItem(variant string, children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"group/menubar-item",
			"focus:bg-accent focus:text-accent-foreground dark:data-[variant=destructive]:focus:bg-destructive/20 not-data-[variant=destructive]:focus:**:text-accent-foreground gap-2 rounded-sm px-2 py-1.5 text-sm [&_svg:not([class*='size-'])]:size-4 flex",
			switch variant {
			case "destructive":
