...
component NavigationMenuTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			...
			"hover:bg-muted focus:bg-muted data-[state=open]:hover:bg-muted data-[state=open]:focus:bg-muted data-[state=open]:bg-muted/50 focus-visible:ring-ring/50 rounded-md px-4 py-2 text-sm font-medium transition-all focus-visible:ring-3 focus-visible:outline-1 disabled:opacity-50 inline-flex h-9 w-max items-center justify-center outline-none disabled:pointer-events-none"
		}
		type="button"
		aria-expanded="false"
		data-state="closed"
...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"focus-visible:ring-ring/50 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex flex-col gap-1.5 rounded-md p-2 text-sm transition-all outline-none focus-visible:ring-3 focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-sm [&_svg:not([class*='text-'])]:text-muted-foreground",
			switch linkVariant {
			case "trigger":
