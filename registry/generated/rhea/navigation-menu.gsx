...
component NavigationMenuTrigger(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			...
			"hover:bg-muted focus:bg-muted data-[state=open]:hover:bg-muted data-[state=open]:focus:bg-muted data-[state=open]:bg-muted/50 focus-visible:ring-ring/30 rounded-2xl px-2.5 py-1.5 text-sm font-medium transition-all focus-visible:ring-3 focus-visible:outline-1 disabled:opacity-50 inline-flex h-9 w-max items-center justify-center outline-none disabled:pointer-events-none"
		}
		type="button"
		aria-expanded="false"
		data-state="closed"
	...
	>
		...
		<icon.ChevronDown
			class={
				"relative top-px ml-1 size-3 transition duration-300 group-data-[state=open]/navigation-menu-trigger:rotate-180"
			}
...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"focus-visible:ring-ring/30 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex flex-col gap-1 rounded-2xl px-2.5 py-1.5 text-sm transition-all outline-none focus-visible:ring-3 focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-xl font-medium [[data-gsxui-slot-navigation-menu-content]_&]:font-normal [[data-gsxui-slot-navigation-menu-content]_&]:p-2 [[data-gsxui-slot-navigation-menu-content]_&]:w-full [&_svg:not([class*='text-'])]:text-muted-foreground",
			switch linkVariant {
			case "trigger":
			...
			switch linkActive { case "true": "focus:bg-muted hover:bg-muted bg-muted/50" default: "outline-none" }
		}
...
component NavigationMenuIndicator(attrs gsx.Attrs) {
	<div
		class={
			"flex top-full z-1 h-1.5 items-end justify-center overflow-hidden pointer-events-none opacity-0 data-[state=visible]:opacity-100"
		}
		data-state="hidden"
		{ attrs... }
