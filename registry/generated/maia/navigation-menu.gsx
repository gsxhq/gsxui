...
component NavigationMenuLink(active bool, variant string, children gsx.Node, attrs gsx.Attrs) {
	...
	<a
		class={
			"focus-visible:ring-ring/50 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex flex-col gap-1.5 rounded-2xl p-3 text-sm transition-all outline-none focus-visible:ring-[3px] focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4 [[data-gsxui-slot-navigation-menu-content]_&]:rounded-xl [&_svg:not([class*='text-'])]:text-muted-foreground",
			switch linkVariant {
			case "trigger":
