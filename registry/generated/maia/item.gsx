...
component Item(variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/item",
			"[a]:hover:bg-muted rounded-2xl border text-sm flex flex-wrap w-full items-center transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors",
			switch variant {
			case "outline":
				"border-border"
			case "muted":
			...
			switch size { case "sm": "gap-3.5 px-3.5 py-3" default: "gap-3.5 px-4 py-3.5" }
		}
		{ attrs... }
...
component ItemTitle(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2 text-sm leading-snug font-medium underline-offset-4 line-clamp-1 w-fit items-center" }
		{ attrs... }
...
component ItemDescription(children gsx.Node, attrs gsx.Attrs) {
	<p
		class={
			"text-muted-foreground text-left text-sm line-clamp-2 font-normal [&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-primary"
		}
		{ attrs... }
