...
component Item(variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			"group/item",
			"[a]:hover:bg-muted rounded-none border text-xs flex flex-wrap w-full items-center transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors",
			switch variant {
			case "outline":
				"border-border"
			case "muted":
...
component ItemTitle(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "gap-2 text-xs font-medium underline-offset-4 line-clamp-1 w-fit items-center" }
		{ attrs... }
