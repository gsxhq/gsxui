...
component Empty(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-4 rounded-none border-dashed p-6 flex flex-col" } { attrs... } data-gsxui-slot-empty>
		{ children }
	</div>
}
...
component EmptyDescription(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "text-xs/relaxed" } { attrs... } data-gsxui-slot-empty-description>
		{ children }
	</div>
}

component EmptyContent(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-2.5 text-xs flex flex-col" } { attrs... } data-gsxui-slot-empty-content>
		{ children }
	</div>
}
