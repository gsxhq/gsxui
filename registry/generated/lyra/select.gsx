...
component SelectTrigger(size string, children gsx.Node, attrs gsx.Attrs) {
	<button
		...
		class={
			...
			"border-input data-placeholder:text-muted-foreground dark:bg-input/30 dark:hover:bg-input/50 focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 gap-1.5 rounded-none border bg-transparent py-2 pr-2 pl-2.5 text-xs transition-colors select-none focus-visible:ring-1 aria-invalid:ring-1 [&_svg:not([class*='size-'])]:size-4 flex",
			switch size { case "sm": "h-7 rounded-none" default: "h-8" }
		}
		{ attrs... }
...
component SelectLabel(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "text-muted-foreground px-2 py-2 text-xs" } { attrs... } data-gsxui-slot-select-label>{ children }</div>
}
...
component SelectItem(value string, selected bool, disabled bool, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			...
			"focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground gap-2 rounded-none py-2 pr-8 pl-2 text-xs [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 flex data-[state=checked]:[&>[data-gsxui-slot-select-item-indicator]]:flex"
		}
		{ attrs... }
...
component SelectSeparator(attrs gsx.Attrs) {
	<div aria-hidden="true" class={ "bg-border -mx-1 h-px" } { attrs... } data-gsxui-slot-select-separator></div>
}
