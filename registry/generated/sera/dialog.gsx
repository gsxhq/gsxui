...
component DialogContent(hideCloseButton bool, children gsx.Node, attrs gsx.Attrs) {
	<dialog
		class={
			"transition-none backdrop:transition-none data-[state=open]:backdrop:animate-in data-[state=closed]:backdrop:animate-out data-[state=closed]:backdrop:fade-out-0 data-[state=open]:backdrop:fade-in-0 backdrop:bg-black/20 backdrop:duration-100 supports-backdrop-filter:backdrop:backdrop-blur-sm bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 ring-foreground/10 open:grid max-w-[calc(100%-2rem)] gap-6 rounded-none p-6 text-sm shadow-md ring-1 duration-100 sm:max-w-md fixed z-50 top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2 outline-none"
		}
		data-state="closed"
		{ attrs... }
...
component DialogTitle(children gsx.Node, attrs gsx.Attrs) {
	<h2 class={ "text-lg font-semibold leading-none uppercase tracking-wider" } { attrs... } data-gsxui-slot-dialog-title>
		{ children }
	</h2>
}
...
component DialogDescription(children gsx.Node, attrs gsx.Attrs) {
	<p
		class={
			"text-muted-foreground *:[a]:hover:text-foreground text-sm *:[a]:underline *:[a]:underline-offset-3 leading-relaxed mt-0.5"
		}
		{ attrs... }
