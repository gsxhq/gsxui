		...
		<InputGroupAddon align="inline-end">
			{ if showTrigger {
				<InputGroupButton
					...
					disabled={disabled}
					class={
						"[[data-gsxui-slot-input-group]:has([data-gsxui-slot-combobox-clear])_&]:hidden [&_svg:not([class*='size-'])]:size-3.5"
					}
					data-gsxui-slot-combobox-trigger
				>
					<icon.ChevronDown class={ "text-muted-foreground size-3.5" } data-gsxui-slot-combobox-trigger-icon/>
				</InputGroupButton>
			} }
			{ if showClear {
...
component ComboboxContent(children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			...
			"transition-none bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 [&>[data-gsxui-slot-input-group]]:bg-transparent max-h-72 min-w-36 overflow-hidden rounded-none shadow-md ring-1 duration-100 [&>[data-gsxui-slot-input-group]]:m-1.5 [&>[data-gsxui-slot-input-group]]:mb-0 [&>[data-gsxui-slot-input-group]]:h-8 [&>[data-gsxui-slot-input-group]]:border-transparent [&>[data-gsxui-slot-input-group]]:focus-within:border-transparent [&>[data-gsxui-slot-input-group]]:px-2.5 [&>[data-gsxui-slot-input-group]]:border-b-input"
		}
		{ attrs... }
...
component ComboboxList(children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		tabindex="-1"
		class={
			"[scrollbar-width:none] [&::-webkit-scrollbar]:hidden max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--gsxui-available-height,9999px)---spacing(9)))] scroll-py-1.5 overflow-y-auto p-1.5 data-empty:p-0"
		}
		{ attrs... }
...
component ComboboxItem(value string, selected bool, children gsx.Node, attrs gsx.Attrs) {
	<div
		...
		class={
			...
			"data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground gap-2.5 rounded-none py-2 pr-8 pl-3 text-sm [&_svg:not([class*='size-'])]:size-3.5 flex data-[state=checked]:[&>[data-gsxui-slot-combobox-item-indicator]]:flex"
		}
		{ attrs... }
...
component ComboboxLabel(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "px-3 py-2 text-xs uppercase tracking-wider font-semibold text-muted-foreground" }
		{ attrs... }
...
component ComboboxSeparator(attrs gsx.Attrs) {
	<div
		aria-hidden="true"
		class={ "bg-border/50 -mx-1.5 my-1.5 h-px" }
		{ attrs... }
