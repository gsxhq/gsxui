...
component CardHeader(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"gap-1.5 rounded-none px-(--card-spacing) grid auto-rows-min items-start has-[[data-gsxui-slot-card-action]]:grid-cols-[1fr_auto] has-[[data-gsxui-slot-card-description]]:grid-rows-[auto_auto]"
		}
		{ attrs... }
...
component CardFooter(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "px-(--card-spacing) flex items-center" } { attrs... } data-gsxui-slot-card-footer>{ children }</div>
}
