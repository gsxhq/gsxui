...
component Toggle(pressed bool, variant string, size string, children gsx.Node, attrs gsx.Attrs) {
	...
	<button
		...
		class={
			"hover:text-foreground aria-pressed:bg-muted focus-visible:border-ring focus-visible:ring-ring/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive disabled:pointer-events-none disabled:opacity-50 gap-1.5 rounded-none text-xs font-semibold uppercase tracking-widest transition-colors [&_svg:not([class*='size-'])]:size-3.5 inline-flex items-center justify-center whitespace-nowrap outline-none focus-visible:ring-[3px] [&_svg]:pointer-events-none [&_svg]:shrink-0",
			switch variant { case "outline": "border-input hover:bg-muted border bg-transparent" default: "bg-transparent" },
			switch size {
			case "sm":
				"h-9 min-w-9 px-4 has-[>svg]:px-3"
			case "lg":
				"h-11 min-w-11 px-8 has-[>svg]:px-5"
			default:
				"h-10 min-w-10 px-6 has-[>svg]:px-4"
			}
		}
		{ attrs... }
