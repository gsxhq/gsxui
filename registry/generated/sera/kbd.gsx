...
component Kbd(children gsx.Node, attrs gsx.Attrs) {
	<kbd
		class={
			"bg-muted text-muted-foreground [[data-gsxui-slot-tooltip-content]_&]:bg-background/20 [[data-gsxui-slot-tooltip-content]_&]:text-background dark:[[data-gsxui-slot-tooltip-content]_&]:bg-background/10 h-5.5 w-fit min-w-5.5 gap-1 rounded-none px-1.5 font-sans text-xs font-medium [&_svg:not([class*='size-'])]:size-3 [[data-gsxui-slot-input-group]_&]:bg-input inline-flex pointer-events-none items-center justify-center select-none"
		}
		{ attrs... }
		data-gsxui-slot-kbd
	>
