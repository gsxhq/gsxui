		...
		<>
			...
			>
				...
				>
					<div
						class={
							"bg-sidebar [[data-gsxui-slot-sidebar-desktop][data-variant=floating]_&]:ring-sidebar-border [[data-gsxui-slot-sidebar-desktop][data-variant=floating]_&]:rounded-none [[data-gsxui-slot-sidebar-desktop][data-variant=floating]_&]:shadow-sm [[data-gsxui-slot-sidebar-desktop][data-variant=floating]_&]:ring-1"
						}
...
component SidebarTrigger(attrs gsx.Attrs) {
	...
	>
		...
		<span class={ "sr-only" } data-gsxui-slot-sidebar-trigger-label>{ i18n.T("Toggle Sidebar") }</span>
	</Button>
}
...
component SidebarInput(attrs gsx.Attrs) {
	<Input class={ "bg-transparent border-input h-8 w-full shadow-none" } { attrs... } data-gsxui-slot-sidebar-input/>
}

component SidebarHeader(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-2 p-2 [--radius:0] flex flex-col" } { attrs... } data-gsxui-slot-sidebar-header>{ children }</div>
}
...
component SidebarContent(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={ "[scrollbar-width:none] [&::-webkit-scrollbar]:hidden gap-2 [--radius:0]" }
		{ attrs... }
...
component SidebarGroup(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "p-2 flex flex-col relative w-full min-w-0" } { attrs... } data-gsxui-slot-sidebar-group>
		{ children }
	</div>
}
...
component SidebarGroupAction(children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			"text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground absolute top-3.5 end-3 [[data-gsxui-slot-sidebar-desktop][data-collapsible=icon]_&]:hidden w-5 rounded-none p-0 focus-visible:ring-2 [&>svg]:size-3.5 flex aspect-square items-center justify-center outline-hidden transition-transform [&>svg]:shrink-0"
		}
		type="button"
		{ attrs... }
...
component SidebarMenuAction(showOnHover bool, children gsx.Node, attrs gsx.Attrs) {
	<button
		class={
			"text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground [[data-gsxui-slot-sidebar-menu-button]:hover~&]:text-sidebar-accent-foreground absolute top-1.5 end-1 aspect-square w-5 rounded-none p-0 [[data-gsxui-slot-sidebar-desktop][data-collapsible=icon]_&]:hidden [[data-gsxui-slot-sidebar-menu-button][data-size=default]~&]:top-2 [[data-gsxui-slot-sidebar-menu-button][data-size=lg]~&]:top-2.5 [[data-gsxui-slot-sidebar-menu-button][data-size=sm]~&]:top-1 focus-visible:ring-2 [&>svg]:size-3.5 md:[&[data-show-on-hover]]:opacity-0 md:[[data-gsxui-slot-sidebar-menu-item]:hover>&[data-show-on-hover]]:opacity-100 md:[[data-gsxui-slot-sidebar-menu-item]:focus-within>&[data-show-on-hover]]:opacity-100 md:[&[data-show-on-hover][data-state=open]]:opacity-100 [[data-gsxui-slot-sidebar-menu-button][data-active]~&[data-show-on-hover]]:text-sidebar-primary-foreground flex items-center justify-center outline-hidden transition-transform [&>svg]:shrink-0"
		}
		type="button"
...
component SidebarMenuBadge(children gsx.Node, attrs gsx.Attrs) {
	<div
		class={
			"text-sidebar-foreground [[data-gsxui-slot-sidebar-menu-button]:hover~&]:text-sidebar-accent-foreground [[data-gsxui-slot-sidebar-menu-button][data-active]~&]:text-sidebar-primary-foreground pointer-events-none absolute end-1 [[data-gsxui-slot-sidebar-desktop][data-collapsible=icon]_&]:hidden flex h-5 min-w-5 rounded-none px-1 text-xs font-medium [[data-gsxui-slot-sidebar-menu-button][data-size=default]~&]:top-1.5 [[data-gsxui-slot-sidebar-menu-button][data-size=lg]~&]:top-2.5 [[data-gsxui-slot-sidebar-menu-button][data-size=sm]~&]:top-1 items-center justify-center tabular-nums select-none"
		}
		{ attrs... }
...
component SidebarMenuSkeleton(showIcon bool, attrs gsx.Attrs) {
	...
	<div class={ "h-8 gap-2 rounded-none px-2 flex items-center" } { attrs... } data-gsxui-slot-sidebar-menu-skeleton>
		{ if showIcon {
			<Skeleton class={ "size-3.5 rounded-none" } data-gsxui-slot-sidebar-menu-skeleton-icon/>
		} }
		<Skeleton
...
component SidebarMenuSubButton(size string, isActive bool, children gsx.Node, attrs gsx.Attrs) {
	<a
		class={
			"text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground [&[data-active]]:bg-sidebar-accent [&[data-active]]:text-sidebar-accent-foreground h-7 gap-2 rounded-none px-3 focus-visible:ring-2 data-[size=md]:text-sm data-[size=sm]:text-xs [&>svg]:size-3.5 flex items-center overflow-hidden [&>span:last-child]:truncate -translate-x-px min-w-0 outline-hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>svg]:shrink-0"
		}
