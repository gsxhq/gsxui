...
component SheetHeader(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-1.5 p-8 flex flex-col" } { attrs... } data-gsxui-slot-sheet-header>{ children }</div>
}

component SheetFooter(children gsx.Node, attrs gsx.Attrs) {
	<div class={ "gap-2 p-8 flex flex-col mt-auto" } { attrs... } data-gsxui-slot-sheet-footer>{ children }</div>
}
